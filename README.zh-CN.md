...
Doctor 会在插件拖垮真实 DSH profile 前发现问题：错误的 package 与 bundle 契约、Host/Client 注入混用、会让 Web 白屏的 Client 服务冲突、浏览器端模块泄漏、不可复现的输出，以及无法通过干净 profile 生命周期验证的构建。整个审计过程保持目标仓库只读。
