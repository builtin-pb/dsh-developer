...
Hard limits include 8 MiB compressed input, 32 MiB decoded input, 4 MiB per frame, 4 MiB per JSONL line (4,194,304 UTF-8 bytes, excluding LF), 25,000 events and a 64 KiB report. The record cap matches the existing decoded-frame capacity: native DSH writes each append batch as a frame, and an ordinary message can occupy most of one. The cap also applies to ignored conversation events and lines spanning concatenated frames, without exporting their bodies. Oversized lines reject the entire inspection with `SESSION_LIMIT`; malformed events also fail inspection. Neither is skipped, even after a completed `turn/end`. Cancellation is checked between bounded reads, frames and event batches. A stable exported log is preferable to a file being actively written.
