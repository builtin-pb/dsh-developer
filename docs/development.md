...
Invalid case files identify the one-based case number, field and expected format before installing the plugin. Numbers in `arguments` and `expected` must be finite and cannot be negative zero: JSON transport would change those values and could produce a misleading pass. Incomplete verification and startup failures show a bounded, credential-checked process excerpt in the normal terminal report. Error headers take priority over long stacks; omissions and withheld streams remain explicit. `--json` retains the structured diagnostic and any larger protected process capture.
