...
On Windows, `verify` and `dev` preserve spaces and shell punctuation in source and temporary-store paths through DSH's native pnpm forwarding. Paths containing `%` or `!` are rejected because the upstream shell can expand them; select a different source or temporary directory.
