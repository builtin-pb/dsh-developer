...
async function probeLane(dshPath, lane) {
  ...
  try {
    ...
    assert.equal(witness.native.operation, 'delegation')
    assert.equal(witness.native.ok, true)
    assert.equal(witness.guard.denied, true)
    assert.match(witness.guard.text, /cannot widen its permission scope/u)
...
test('corrects delegated schemas through exact release and preview DSH runtimes', {
  skip: !enabled,
  timeout: 150_000,
}, async () => {
  ...
  assert.equal(preview.version, '0.1.2-alpha.2')
