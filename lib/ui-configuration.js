...
export function uiSetupCommandInstruction(installCli = false) {
  const command = 'ui-setup' + (installCli ? ' --install-cli' : '')
  return 'In a DSH agent shell, run node "$DSH_DEVELOPER_BIN" ' + command + ' (POSIX) or node "$env:DSH_DEVELOPER_BIN" '
    + command + ' (PowerShell); outside DSH, run node bin/dsh-developer.js ' + command + ' from the dsh-developer checkout root.'
}
export const UI_SETUP_INSTRUCTION = uiSetupCommandInstruction()
  + ' Append --install-cli to explicitly install @playwright/cli@0.1.18. Then restart DSH and rerun dsh_developer {"operation":"ui"}.'
