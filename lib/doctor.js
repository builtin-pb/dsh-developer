...
export async function doctorPlugin(source, options = {}) {
  ...
  await attempt(target, 'source.ordinary-tree', async () => {
    tree = await scanOrdinaryTree(absolute, { signal: options.signal, excludeDependencies: true })
    files = mapTreeEntries(tree)
    target.fingerprint = tree.fingerprint
    return {
      message: 'Plugin source contains only bounded, portable, case-safe UTF-8 files and no detected secrets; installed dependencies stay outside the snapshot.',
      evidence: {
  ...
  if (tree) {
    await attempt(target, 'verification.freshness', async () => {
      const fresh = await scanOrdinaryTree(absolute, { signal: options.signal, excludeDependencies: true })
