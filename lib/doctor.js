...
function validateDshEntrypoint(files, pluginName) {
  const patch = requiredFile(files, 'cordis.patch.yml')
  assert(
    new RegExp('^\\s*- id:\\s*' + pluginName.replace(/[$^.*+?{}()|[\]\\]/gu, '\\$&') + '\\s*$', 'mu').test(patch),
    'INVALID_DSH_PATCH',
    'cordis.patch.yml must insert the plugin id "' + pluginName + '".',
  )
  assert(/^\s*name:\s*['"]?\.\/index\.js['"]?\s*$/mu.test(patch), 'INVALID_DSH_PATCH', 'DSH patch must load the real "./index.js" entry.')
...
export async function doctorPlugin(source, options = {}) {
  ...
  if (files) {
    ...
    await attempt(target, 'dsh.entrypoint', async () => {
      ...
      validateDshEntrypoint(files, codexValue.name)
  ...
  if ((reproducible || isProductSource) && packageValue && codexValue && runtime) {
    await attempt(target, 'dsh.clean-profile-lifecycle', async () => {
      ...
      return { message: 'Clean-profile DSH install, discovery, and uninstall passed with pnpm offline and lifecycle scripts disabled.' }
    }, {
