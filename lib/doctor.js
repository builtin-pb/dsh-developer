...
import { fingerprintFileMap, mapTreeEntries, scanSourceAuditTree } from './files.js'
import {
  checkDshVersion,
...
import { inspectWebRouteAuth, inspectExecutableModuleMetadata } from './web-route-audit.js'
...
function requiredFile(files, path) {
  const content = files.get(path)
  if (content === undefined && files.has(path)) {
    throw new DshDeveloperError('BINARY_FILE', 'Required source file "' + path + '" must be UTF-8 text.', { path })
  }
  if (content === undefined) throw new DshDeveloperError('MISSING_FILE', 'Required file "' + path + '" is missing.', { path })
  return content
}
...
// Static, non-executing recognition of a callable `apply` export. It reuses the
// repository's Babel-based module analysis so comments, strings, shadowed bindings,
// re-exports, and reassigned or non-callable bindings cannot satisfy the check.
function exportsCallableApply(source, sourcePath) {
  const metadata = inspectExecutableModuleMetadata(source, { sourcePath })
  if (!metadata?.parsed) return false
  const exported = metadata.exports?.find((item) => item.name === 'apply')
  return exported?.descriptors?.length === 1 && exported.descriptors[0].kind === 'function'
}
...
function validatePackage(files, options = {}) {
  ...
  if (options.generatedBundle) {
    ...
    const expectedNodeRange = options.productSource
      ? '^22.18.0 || >=24.11.0'
      : '>=22.18'
    assert(
      value.engines?.node === expectedNodeRange,
      'INVALID_PACKAGE',
      options.productSource
        ? 'dsh-developer package.json must match the pinned Babel Node.js engine range.'
        : 'Generated package.json must require Node.js >=22.18.',
    )
  }
  return value
}
...
function validateCodexManifest(files, options = {}) {
  ...
  if (options.productSource) {
    assert(value.version === options.packageVersion, 'INVALID_CODEX_MANIFEST', 'dsh-developer Codex and package versions must match.')
  } else if (options.generatedBundle) {
...
function validateSkill(files, pluginName) {
  ...
  if (match[3] !== undefined) {
    const whenToUse = JSON.parse('"' + match[3] + '"')
    assert(whenToUse.length > 0 && whenToUse.length <= 1024, 'INVALID_SKILL', 'Skill whenToUse must be 1-1024 characters.', { path })
  }
...
function validateDshEntrypoint(files, pluginName, packageName, options = {}) {
  ...
  if (!options.generatedBundle) {
    ...
    if (mountedId) {
      assert(
        exportsCallableApply(entry, entryPath),
        'INVALID_DSH_ENTRY',
...
export async function doctorCreator(source, options = {}) {
  ...
  if (snapshot !== undefined) target.snapshot = snapshot
...
export async function doctorPlugin(source, options = {}) {
  ...
  await attempt(target, 'source.ordinary-tree', async () => {
    tree = await scanSourceAuditTree(absolute, { signal: options.signal })
    files = mapTreeEntries(tree)
    target.fingerprint = tree.fingerprint
    return {
      message: 'Plugin source contains bounded, portable, case-safe UTF-8 files and recognized binary assets with no detected credential markers; installed dependencies stay outside the snapshot.',
      evidence: {
        files: tree.fileCount,
        entries: tree.treeEntries,
        bytes: tree.bytes,
        budget: tree.budget,
        excludedDirectories: tree.excludedDirectories,
        binaryAssets: tree.entries.filter((entry) => entry.mediaType),
        fingerprint: tree.fingerprint,
      },
    }
  }, {
    recovery: 'Remove links, special files, unsupported binaries, case collisions, oversized content, credentials, or concurrent writers.',
  })
  ...
  if (files) {
    ...
    await attempt(target, 'manifest.codex-plugin', async () => {
      ...
      codexValue = validateCodexManifest(files, { generatedBundle: generatedBundleContract,
        productSource: isProductSource, packageVersion: packageDocument?.version })
    ...
    await attempt(target, 'web.raw-route-auth', async () => {
      if (!dshValue?.entryPath) {
        return {
          status: 'SKIP',
          blocking: false,
          message: 'Raw Web-route audit requires a valid package entry.',
        }
      }
      const evidence = inspectWebRouteAuth(files, { entryPath: dshValue.entryPath })
      const incomplete = evidence.coverage.incompletePaths.length > 0
      if (evidence.rawRoutes.length > 0) {
        return {
          status: 'WARN',
          blocking: false,
          message: 'Raw plugin-owned Web route registration sits outside the upstream connection registration boundary. Doctor is surfacing an authentication-intent review, not claiming every raw route is unsafe.'
            + (incomplete ? ' Static route coverage is incomplete for one or more reachable modules.' : ''),
          evidence,
          recovery: 'For private or API traffic, register through the upstream connection service and verify the exact release and preview lanes. If the raw route is intentionally public, retain it only with documented public intent and verification that it exposes no private data or privileged action.'
            + (incomplete ? ' Repair malformed or unsupported reachable source before relying on route absence.' : ''),
        }
      }
      if (incomplete) {
        return {
          status: 'WARN',
          blocking: false,
          message: 'Static route coverage is incomplete for one or more package-entry-reachable modules; Doctor is not claiming clean route absence.',
          evidence,
          recovery: 'Repair malformed or unsupported reachable source, then rerun Doctor before relying on route absence.',
        }
      }
      if (evidence.connectionRoutes.length > 0) {
        return {
          status: 'PASS',
          blocking: false,
          message: 'Observed plugin-owned channels registered through the upstream connection service; no direct raw registration was observed in the package-entry-reachable static closure. Authentication depends on the exact runtime and configuration; this static observation does not establish it.',
          evidence,
          recovery: 'Verify registration support and unauthenticated-request behavior on the exact deployed runtime. Connection APIs and authentication differ across DSH versions.',
        }
      }
      return {
        status: 'SKIP',
        blocking: false,
        message: 'No direct raw Web or connection registration was observed in the package-entry-reachable static closure; local absence is not a safety proof.',
        evidence,
      }
    }, { blocking: false })
    ...
    await attempt(target, 'compatibility.upstream-attachments', async () => {
      ...
      if (contract.unparsedDeclarations.length > 0 || contract.invalidValues.length > 0
          || !contract.contextCoverage.complete || !contract.activationCoverage.complete) {
        throw new DshDeveloperError(
          contract.injectionValidity.ok ? 'INCOMPLETE_UPSTREAM_ATTACHMENTS' : 'UNSCOPED_INJECT_CONTRACT',
          describeHostAttachmentFailure(contract),
          contract,
        )
      }
      const uncovered = [
      ...
      const opaqueRuntimePaths = references.coverage.unparsedModuleClosure ?? []
      return uncovered.length === 0 && opaqueRuntimePaths.length === 0
        ? {
        ...
        : uncovered.length === 0
          ? {
              status: 'WARN',
              blocking: false,
              message: 'Cordis activation attachments are proven, but non-activation runtime loader coverage is incomplete.',
    ...
    await attempt(target, 'packaging.reproducible', async () => {
      if (!isProductSource && tree.excludedDirectories.length > 0
          && (options.requireGenerated || hasProvenance || hasGeneratedManifest)) {
        throw new DshDeveloperError(
          'PROVENANCE_DEPENDENCIES_EXCLUDED',
          'A generated release cannot execute with dependency directories outside its reproducible snapshot.',
          { excludedDirectories: tree.excludedDirectories },
        )
      }
  ...
  const runtime = await runtimeCheck(target, options)
  if (options.runtime === 'skip') {
    skip(target, 'tests.generated-smoke', 'Static audits do not execute plugin code, including reproducible generated bundles.')
  } else if (reproducible && packageValue && codexValue) {
  ...
  if (tree) {
    await attempt(target, 'verification.freshness', async () => {
      const fresh = await scanSourceAuditTree(absolute, { signal: options.signal })
      ...
      return {
        ...
        evidence: { fingerprint: fresh.fingerprint, budget: fresh.budget },
      }
    })
  } else {
  ...
  if (packageValue) {
    target.plugin = {
      name: codexValue?.name ?? dshValue?.pluginName ?? packageValue.name,
