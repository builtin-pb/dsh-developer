    ...
    await attempt(target, 'web.client-bundle', async () => {
      ...
      if (!evidence.lanes.preview.ok) {
        return {
          status: 'WARN',
          blocking: false,
          message: 'The Web bundle is valid for the blocking release but requests modules absent from the reviewed preview module table.',
          evidence,
        }
      }
      return {
        message: 'The DSH Web bundle has a valid client declaration, classic-script handoff, and closed browser module boundary.',
        evidence,
      }
    }, {
      recovery: 'Rebuild the exported ./client classic script, register the exact package id, keep Node and file imports out, and declare non-platform client modules in dsh.client.external.',
    })
