...
function packageEntryPath(value) {
  let entry = value.main
  if (entry === undefined) {
    const rootExport = value.exports?.['.']
    if (typeof rootExport === 'string') entry = rootExport
    else if (rootExport && typeof rootExport === 'object' && !Array.isArray(rootExport)) entry = rootExport.default
  }
  return packagePath(entry, 'package.json main or exports["."].default')
}
...
function validateDshEntrypoint(files, pluginName, packageName, options = {}) {
  ...
  if (!options.generatedBundle) {
    ...
    return { pluginName: mountedId ?? packageName, mounted: Boolean(mountedId), entryPath, patchPath }
  }
  ...
  return { pluginName, mounted: true, entryPath, patchPath }
}
  ...
  if (files) {
    ...
    await attempt(target, 'dsh.entrypoint', async () => {
      ...
      dshValue = validateDshEntrypoint(files, codexValue?.name ?? packageValue.name, packageValue.name, {
        ...
        entryPath: packageEntryPath(packageValue),
