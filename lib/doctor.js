    ...
    await attempt(target, 'web.raw-route-auth', async () => {
      ...
      if (evidence.rawRoutes.length > 0) {
        return {
          status: 'WARN',
          blocking: false,
          message: 'Raw plugin-owned Web route registration sits outside the authenticated connection registration boundary. Doctor is surfacing an authentication-intent review, not claiming every raw route is unsafe.'
      ...
      if (evidence.connectionRoutes.length > 0) {
        return {
          status: 'PASS',
          blocking: false,
          message: 'Observed plugin-owned channels registered through the authenticated connection boundary; no direct raw registration was observed in the package-entry-reachable static closure. That local absence is not a safety proof.',
          evidence,
        }
      }
      return {
