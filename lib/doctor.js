...
export async function doctorPlugin(source, options = {}) {
  ...
  await attempt(target, 'source.ordinary-tree', async () => {
    tree = await scanOrdinaryTree(absolute, { signal: options.signal, excludeDependencies: true, allowBinaryAssets: true })
  ...
  if (tree) {
    await attempt(target, 'verification.freshness', async () => {
      const fresh = await scanOrdinaryTree(absolute, { signal: options.signal, excludeDependencies: true, allowBinaryAssets: true })
