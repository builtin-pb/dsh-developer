...
const UI_PREREQUISITE_DIAGNOSTICS = new Set([
  'UI_CONFIG_INVALID',
  'UI_SETUP_NPM_MISSING',
  'UI_SETUP_INSTALL_FAILED',
  'UI_AGENT_REQUIRED',
  'UI_BROWSER_INVALID',
...
function hookConfigAction() {
  return instruction(
    'hook-doctor.resolve-config',
    'The admitted hook configuration is outside the exact reviewed static subset.',
    'explicit-source-edit',
    'source-change',
    { relation: 'blocks', target: 'doctor-pass' },
    'repair-hook-config',
    'Using only the redacted issue codes and handler classes, repair the hook configuration only within existing source-edit authority; otherwise obtain explicit edit approval before any change. Keep every handler within the exact reviewed static subset, then rerun hook-doctor on the same reviewed DSH lane. Do not alter package bytes, execute or expand a handler, or claim activation; never use preview evidence as release proof.',
  )
}
...
function preserveApplyRecoveryAction() {
  return instruction(
    'cell.preserve-apply-recovery',
    'Cell provider cleanup, source, stage, or transaction recovery evidence is unverified, so destructive cleanup is unsafe.',
    'read-only',
    'inspection',
    { relation: 'blocks', target: 'cell-capacity' },
    'preserve-apply-recovery',
    'Preserve source, staging, provider identity, and transaction recovery evidence unchanged. Do not call cell-discard or retry Apply, and never delete, move, rename, or reconstruct any reported path. Have an authorized operator inspect only the bounded failure code, owned provider identity, and controller-owned recovery markers, then continue only through an explicit recovery procedure that proves source state and provider cleanup.',
  )
}
...
function uiPrerequisiteAction() {
  return instruction(
    ...
    'configure-pinned-ui-prerequisites',
    'Run dsh-developer ui-setup; use --install-cli explicitly if @playwright/cli@0.1.18 is missing. Restart DSH, then rerun dsh_developer {"operation":"ui"}. Do not discover or install them automatically during runtime or admission, copy diagnostic paths, add credentials, or relax loopback isolation.',
  )
}
...
function uiAdmissionAction() {
  return instruction(
    ...
    'admit-protected-ui-provider',
    'Keep browser execution, file transfer, and non-loopback navigation denied. Run dsh-developer ui-setup for the native route, restart DSH, then rerun dsh_developer {"operation":"ui"}; if the tool remains absent, check the calling Agent tool scope before any UI action.',
  )
}
...
export function deriveNextActions(context = {}) {
  ...
  const hookDoctor = report?.kind === 'doctor-hook-bridge' || operationName === 'hook-doctor'
  if (hookDoctor) {
    if (checkFailed(checks, 'source.authority')) return deepFreeze([])
    if (report?.lane?.status === 'reviewed-absent') return deepFreeze([])
    if (checkFailed(checks, 'lane.identity')
        || checkFailed(checks, 'bridge.bytes')) {
      return deepFreeze([laneAction()])
    }
    if (report?.config?.status === 'inspected' && report.ok === false) {
      return deepFreeze([hookConfigAction()])
    }
    return deepFreeze([])
  }
  ...
  if (!['project', 'knowledge', 'session'].includes(operationName) && hasLaneBlocker(report, checks, code)) offer(30, laneAction())
