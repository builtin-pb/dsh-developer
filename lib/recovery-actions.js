...
function hookConfigAction() {
  return instruction(
    ...
    { relation: 'blocks', target: 'doctor-pass' },
    'repair-hook-config',
    'Using only the redacted issue codes and handler classes, repair the already-authorized hook configuration so every handler is within the exact reviewed static subset, then rerun hook-doctor on the same reviewed DSH lane. Do not alter package bytes, execute or expand a handler, claim activation, or use preview evidence as release proof.',
  )
}
...
export function deriveNextActions(context = {}) {
  ...
  if (hookDoctor) {
    ...
    if (report?.lane?.status === 'unreviewed'
        || checkFailed(checks, 'lane.identity')
