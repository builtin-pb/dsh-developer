...
function uiPrerequisiteAction() {
  return instruction(
    ...
    'configure-pinned-ui-prerequisites',
    UI_SETUP_INSTRUCTION + ' Do not discover or install them automatically during runtime or admission, copy diagnostic paths, add credentials, or relax loopback isolation.',
  )
}
...
function uiAdmissionAction() {
  return instruction(
    ...
    'admit-protected-ui-provider',
    'Keep browser execution, file transfer, and non-loopback navigation denied. ' + UI_SETUP_INSTRUCTION
      + ' If the tool remains absent, check the calling Agent tool scope before any UI action.',
  )
}
