...
// Inspect the selected consumer's portable dependency graph. NODE_PATH and
// global search locations may belong to the DSH process running the inspection,
// rather than the installation being inspected.
export function localPackageSearchPaths(consumer, packageName) {
  return (createRequire(consumer.manifestPath).resolve.paths(packageName) ?? [])
    .filter(path => path === join(dirname(path), 'node_modules')
      && !escapesRoot(dirname(path), consumer.root))
}

export async function locateInstalledDshPackage(dshPackage, packageName) {
  if (!dshPackage) return undefined
  for (const path of localPackageSearchPaths(dshPackage, packageName)) {
    const candidate = join(path, ...packageName.split('/'))
    try {
    ...
    // Package-manager links are valid, but the nearest package shadows later
    // copies even when its own metadata is missing or invalid. Read its actual
    // manifest; an exports map may hide or redirect the package.json subpath.
    const root = await realpath(candidate).catch(() => undefined)
    if (!root) {
      throw new DshDeveloperError('DSH_PACKAGE_INVALID', 'A local package entry cannot be resolved; its installation is incomplete.', {
        path: candidate, package: packageName,
      })
    }
    const manifestPath = join(root, 'package.json')
    const value = await readJson(manifestPath)
    if (value?.name !== packageName) {
      throw new DshDeveloperError('DSH_PACKAGE_INVALID', 'A local package entry lacks a matching ordinary package.json; its presence cannot be classified.', {
        path: manifestPath, package: packageName,
      })
    }
    return {
      root,
