 ...
 */
export async function inspectDshKnowledge({ dshPath, upstreamRoot, topic, packageName, consumerRoot, signal } = {}) {
  cancelled(signal)
  ...
  for (const [name, value] of Object.entries({ dshPath, upstreamRoot, consumerRoot })) {
  ...
  if ([...report.missing, ...(report.development?.missing ?? [])].some((item) => item.reason.includes('limit'))) report.warnings.push('An inspection limit was reached. Select one topic for a narrower lookup; omissions are listed below.')
  cancelled(signal)
  return report
}
