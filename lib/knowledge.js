...
import { localPackageSearchPaths } from './dsh-installation.js'
...
const ROUTES = {
  ...
  ui: {
    ...
    owners: ['ui-renderer', 'ui-slots', 'client-modules', 'gateway', 'connection'],
...
const OWNERS = {
  ...
  'client-modules': ['@deepseek-ai/dsh-client-modules', 'packages/client/modules'],
...
async function developmentPeers(resolvePackage, seed, signal) {
  ...
  while (queue.length) {
    ...
    if (!seen.has(identity)) {
      ...
      seen.add(identity)
      if (!pkg?.value || !version || version !== pkg.value.version) {
        missing.push({ name: request.name, ...(request.consumer ? { consumer: request.consumer } : {}),
          reason: pkg?.reason ?? 'exact-installed-version-unavailable' })
        continue
      }
...
async function locateKnowledgePackage(state, consumer, name, packages, allowNew) {
  ...
  for (const path of paths) {
    ...
    // Read the actual manifest regardless of package.json export visibility.
    // Identity, peer closure and evidence all share the same byte budget.
    const manifest = await readManifest(state, root, 'package.json', LIMITS.identityManifestBytes)
    if (manifest.reason === 'total-read-limit') return { reason: manifest.reason }
    if (manifest.value?.name !== name) return undefined
    const pkg = { root, manifestPath: manifest.path, value: manifest.value }
...
async function readManifest(state, root, name = 'package.json', maxBytes = LIMITS.metadataBytes) {
  const file = await readText(state, root, name, maxBytes)
...
// Source selection uses the same bounded reader as evidence. Do not run project
// inspection here: its script/toolchain metadata has a separate read budget.
async function locateCheckout(state, source, sourceRoot) {
  const requested = await realpath(resolve(sourceRoot ?? process.cwd(), source)).catch(() => null)
  const boundary = sourceRoot === undefined ? undefined : await realpath(sourceRoot).catch(() => null)
  const invalid = reason => new DshDeveloperError('KNOWLEDGE_UPSTREAM_INVALID',
    'Select a DSH checkout, or a package/source file within that checkout. Native selection must stay inside the Agent workspace; open the containing checkout as the workspace. '
      + 'For installed DSH packages, omit source (CLI --upstream) and select packageName (CLI --package) instead.', { reason })
  if (!requested) throw invalid('not-found')
  if (boundary === null || (boundary && (!(await lstat(boundary)).isDirectory() || !inside(boundary, requested)))) {
    throw invalid('outside-workspace')
  }
  const info = await lstat(requested)
  if (!info.isDirectory() && !info.isFile()) throw invalid('not-regular-file')
  let root = info.isDirectory() ? requested : dirname(requested)
  let selected
  for (let depth = 0; depth < LIMITS.ancestorDirectories; depth += 1) {
    cancelled(state.signal)
    const manifest = await readManifest(state, root)
    if (manifest.reason && manifest.reason !== 'not-found') throw invalid(manifest.reason)
    if (manifest.value?.name === '@deepseek-ai/dsh-root') {
      if (!smallString(manifest.value.version)) throw invalid('package-identity')
      if (selected && (typeof selected.manifest.value.name !== 'string' || !PACKAGE_NAME.test(selected.manifest.value.name)
          || !safeRepositoryDirectory(relative(root, selected.root).split(sep).join('/')))) {
        throw invalid('source-package-identity')
      }
      return { root, manifest, selected, requested, file: info.isFile() ? requested : null }
    }
    if (!selected && manifest.value) selected = { root, manifest }
    if (root === boundary || dirname(root) === root || await lstat(join(root, '.git')).catch(() => null)) {
      throw invalid('checkout-not-found')
    }
    root = dirname(root)
  }
  throw invalid('ancestor-search-limit')
}

async function locateKnowledgeDsh(state, invocation) {
  const entry = invocation.prefixArgs.find(value => isAbsolute(value) && /\.[cm]?js$/iu.test(value))
  if (!entry) return undefined
  for (let root = dirname(entry), depth = 0; depth < 8; depth++, root = dirname(root)) {
    const manifest = await readManifest(state, root, 'package.json', LIMITS.identityManifestBytes)
    if (manifest.reason && manifest.reason !== 'not-found') {
      throw new DshDeveloperError('DSH_PACKAGE_INVALID', 'The selected DSH package metadata could not be inspected.', { reason: manifest.reason })
    }
    if (manifest.value?.name === '@deepseek-ai/dsh') return { root, manifestPath: manifest.path, value: manifest.value }
...
async function discoverCode(state, context) {
  ...
  tests.sort()
  if (context.preferredPath) {
    // DSH's foo.client.spec.ts and foo.host.spec.ts both accompany foo.ts.
    // Name matching is a navigation hint, not a claim of behavior coverage.
    const stem = file => basename(file).replace(/\.(?:(?:client|host)\.)?(?:(?:test|spec)\.)?[cm]?[jt]sx?$/u, '')
    const selected = stem(context.preferredPath)
    tests.sort((a, b) => Number(stem(b) === selected) - Number(stem(a) === selected))
  }
  // The explicit file wins one of the existing excerpt slots. It may be deeper
  // than discovery, but cannot expand evidence into fixtures or dependencies.
  const preferred = context.preferredPath
  if (preferred && /^(?:src|tests|test|__tests__)\//u.test(preferred) && CODE.test(preferred)
      && !DECLARATION.test(preferred) && preferred.split('/').every(part => !OMIT.test(part))) {
    const candidates = TEST.test(preferred) || /(?:^|\/)(?:tests?|__tests__)\//u.test(preferred) ? tests : sources
    const index = candidates.indexOf(preferred)
    if (index !== -1) candidates.splice(index, 1)
    candidates.unshift(preferred)
  }
  if (sources.length > LIMITS.sourcesPerPackage) missing(state, context, 'src', 'source', 'source-excerpt-limit')
  if (tests.length > LIMITS.testsPerPackage) missing(state, context, 'tests', 'test', 'test-excerpt-limit')
 ...
 * and excerpt hints. A selected source package supplies its own identity and
 * directory. Otherwise installed repository.directory or known owner mappings
 * locate source owners; no checkout-wide package search is performed.
 * `consumerRoot` selects the physical package from which `packageName` is
 * imported. It requires an installed lookup and packageName; no graph search
 * substitutes another consumer or version. Omitted consumerRoot uses DSH.
 ...
 * `upstreamRoot` accepts a checkout, subpackage or file; sourceRoot confines
 * source selection and ancestor discovery to the native Agent workspace.
 */
export async function inspectDshKnowledge({ dshPath, upstreamRoot, sourceRoot, topic, packageName, consumerRoot, signal } = {}) {
  cancelled(signal)
  ...
  for (const [name, value] of Object.entries({ dshPath, upstreamRoot, sourceRoot, consumerRoot })) {
  ...
  if (consumerRoot !== undefined && (packageName === undefined || (upstreamRoot !== undefined && dshPath === undefined))) {
    throw new DshDeveloperError('KNOWLEDGE_CONSUMER_INVALID', 'consumerRoot requires packageName and an installed DSH lookup.')
  }
  const topics = [topic ?? 'tool']
  const report = {
    kind: 'dsh-knowledge', ok: true, schemaVersion: 1, topics, packageName: packageName ?? null,
    installed: null, upstream: null, development: null, resolution: null, versionMismatch: null, packageVersionMismatches: [],
  ...
  const checkout = upstreamRoot === undefined ? undefined : await locateCheckout(state, upstreamRoot, sourceRoot)
  if (checkout?.selected) {
    const name = checkout.selected.manifest.value.name
    if (packageName !== undefined && packageName !== name) {
      throw new DshDeveloperError('KNOWLEDGE_PACKAGE_CONFLICT',
        'The selected source package differs from packageName. Select the checkout root to inspect another package.')
    }
    packageName = name
  ...
  if (dshPath !== undefined || upstreamRoot === undefined) {
    ...
    cancelled(signal)
    const dsh = await locateKnowledgeDsh(state, invocation)
    cancelled(signal)
    ...
    if (consumerRoot !== undefined) {
      const root = await realpath(resolve(consumerRoot)).catch(() => undefined)
      const manifest = root ? await readManifest(state, root, 'package.json', LIMITS.identityManifestBytes) : undefined
      if (!manifest?.value || !smallString(manifest.value.name)) {
        throw new DshDeveloperError('KNOWLEDGE_CONSUMER_INVALID', 'Select a package directory with an ordinary named package manifest.', {
          reason: manifest?.reason ?? 'package-identity',
        })
      }
      basis = { root, manifestPath: manifest.path, value: manifest.value }
    }
    report.resolution = { consumerRoot: basis.root, consumerManifestPath: basis.manifestPath,
      consumerName: basis.value.name, consumerVersion: smallString(basis.value.version) }
    if (basis === dsh) installedPackages.set('@deepseek-ai/dsh', dsh)
    const consumerPackages = new Map([[basis.manifestPath, installedPackages]])
    const physicalPackages = new Map([[dsh.root, dsh]])
    physicalPackages.set(basis.root, basis)
    const resolvePackage = async (name, consumer = basis, allowNew = true) => {
      cancelled(signal)
    ...
    for (const name of new Set(['@deepseek-ai/dsh', ...owners.map(owner => owner.name)])) {
      ...
      const pkg = name === '@deepseek-ai/dsh' && !selected.length ? dsh : await resolvePackage(name)
      cancelled(signal)
      const context = { origin: 'installed', root: basis.root, packageName: name, topics: selected }
      if (!pkg?.value) {
        missing(state, context, 'package.json', 'package', pkg?.reason ?? 'package-not-installed')
        continue
      }
    ...
    if (packageName !== undefined && !checkout?.selected) {
  ...
  if (upstreamRoot !== undefined) {
    const { root, manifest, selected, requested } = checkout
    if (selected) owners[0].directory = relative(root, selected.root).split(sep).join('/')
    report.upstream = { root, version: manifest.value.version, manifestPath: manifest.path,
      manifestSha256: manifest.sha256, requested, ...await checkoutCommit(state, root), workingTree: 'not-verified-clean', packages: [] }
    ...
    for (const owner of owners) {
      ...
      const pkg = selected?.manifest ?? await readManifest(state, root, directory + '/package.json')
      ...
      await inspectPackage(state, { ...context, root: packageRoot, packageVersion: version,
        preferredPath: selected && checkout.file ? relative(packageRoot, checkout.file).split(sep).join('/') : undefined }, pkg.value)
    }
  ...
  if ([...report.missing, ...(report.development?.missing ?? [])].some((item) => item.reason.includes('limit'))) report.warnings.push('An inspection limit was reached; omissions are listed below. Select a source/test file or inspect omitted files directly for the next lookup.')
  cancelled(signal)
  return report
}
...
export function formatDshKnowledgeReport(report) {
  ...
  if (report.resolution) lines.push('Resolve imports from: ' + report.resolution.consumerName + ' — ' + report.resolution.consumerRoot)
