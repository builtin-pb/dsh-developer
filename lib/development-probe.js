...
export function apply(ctx) {
  ...
  const start = () => { void Promise.resolve().then(pending).catch(async error => {
    ...
    await writeFile(process.env.DSH_DEVELOPER_CASE_RESULT,
      JSON.stringify({ ok: false, error: String(error?.message ?? error).slice(0, 2048), cases: [] }), { flag: 'wx', mode: 0o600 }).catch(() => {})
    requestExit(1)
  }) }
  // Current DSH publishes tools before the whole application has finished
  // starting. Exiting in that window can interrupt boot or conceal a sibling
  // failure. Older launchers have no readiness service; retain their native
  // registration/exit checks without claiming full startup evidence.
  const ready = ctx.get('appReady')
  if (ready) ctx.effect(() => ready.onReady(start), 'wait for development startup')
  else start()
}
