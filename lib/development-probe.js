...
export function observeToolCase(item, result, index) {
  ...
  if (item.isError === true && item.errorContains !== undefined
      && !result.content.some(block => block.type === 'text' && typeof block.text === 'string'
        && block.text.includes(item.errorContains))) failures.push('error-message-mismatch')
  ...
  for (const [key, value] of Object.entries({
    ...
    ...(item.errorContains !== undefined ? { errorContains: item.errorContains } : {}),
...
export function apply(ctx) {
  ...
  let spec, handle, agent, error, cleanupError, privateKeyOutput = false, activeCase = null, phase = 'startup'
  const diagnostic = failure => {
    const raw = String(failure?.message ?? failure)
