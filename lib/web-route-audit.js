...
// Shared by module imports, call descriptors, callback sites and context transfers.
export const EXECUTABLE_MODULE_EDGE_LIMIT = 8192
...
function registerTargetWrites(path, writePath) {
  const target = unwrapExpressionPath(path)
  if (!target?.node) return
  if (target.isIdentifier?.()) {
    runtimeBinding(target, target.node.name)?.reassign(writePath)
  } else if (target.isArrayPattern?.()) {
    for (const element of target.get('elements')) registerTargetWrites(element, writePath)
  } else if (target.isObjectPattern?.()) {
    for (const property of target.get('properties')) {
      registerTargetWrites(property.get(property.isRestElement?.() ? 'argument' : 'value'), writePath)
    }
  } else if (target.isAssignmentPattern?.()) {
    registerTargetWrites(target.get('left'), writePath)
  } else if (target.isRestElement?.()) {
    registerTargetWrites(target.get('argument'), writePath)
  }
  // Writing a member changes that property, not the binding of its object.
}
...
function exactConnectionRoute(chain, call) {
  return chain.parts.length === 3 && chain.parts[0].name === 'connection'
    && ((chain.parts[1].name === 'rpc' && chain.parts[2].name === 'handle')
      || (chain.parts[1].name === 'fetch' && chain.parts[2].name === 'register')) && Boolean(call)
}

function contextForwardingDescriptor(reference, recordByNode) {
  const argument = outwardExpressionPath(reference)
  const call = argument.parentPath
  if (!call?.isCallExpression?.() || call.node.optional === true
      || call.node.arguments.length !== 1 || call.node.arguments[0] !== argument.node) return undefined
  const callee = unwrapExpressionPath(call.get('callee'))
  ...
  return record ? { kind: 'local', functionId: record.id } : undefined
}
...
function contextReferenceResult(
  ...
  for (const path of references) {
    ...
    if (!chain.dynamic && chain.parts.length === 0) {
      const descriptor = contextForwardingDescriptor(path, recordByNode)
      if (descriptor) {
        addDescriptor(callCollector, 'context-forwarding', forwardings, descriptor)
        continue
      }
    }
    if (chain.dynamic || chain.parts.length === 0) {
  ...
  return {
    context: { complete: complete && forwardings.length === 0, properties, serviceLookups, injections },
    forwarding: { localComplete: complete, calls: forwardings },
  }
}
...
function routeSummaries(record, bindingReferences, incompleteBindings, withContextPaths) {
  ...
  for (const reference of references) {
    ...
    if (executionFunctionParent(reference)?.node !== record.path.node) continue
    ...
    if (chain.parts[0].name === 'connection') {
      ...
      const fetch = chain.parts[1].name === 'fetch'
      const route = fetch ? literalObjectFields(call.node.arguments[0]).path
        : staticString(call.node.arguments[0])
      connectionRoutes.push({
        sourcePath: record.sourcePath,
        line: reference.node.loc?.start.line ?? 1,
        call: fetch ? 'ctx.connection.fetch.register' : 'ctx.connection.rpc.handle',
        routePath: route,
...
function executionFunctionParent(path) {
  let owner = path.getFunctionParent?.()
  // A computed method name runs while the containing object/class is created,
  // outside that method's body. Nested functions inside the key retain their
  // own execution boundary; their invocation is classified separately.
  while (owner?.node.computed && nodeWithin(path.node, owner.node.key)) {
...
function edgeExecutionOwner(path, recordByNode) {
  const owner = executionFunctionParent(path)
...
function exactCreateRequireResolveReference(reference) {
  ...
  let resolver = outwardExpressionPath(member)
  const pathsMember = resolver.parentPath
  // Node's resolve.paths lists search directories without loading a module.
  // Admit only the direct call, never an escaped or replaced loader method.
  if (pathsMember?.isMemberExpression?.() && pathsMember.node.optional !== true
      && pathsMember.node.object === resolver.node && staticMemberName(pathsMember) === 'paths') {
    resolver = outwardExpressionPath(pathsMember)
  }
...
function analyzeSource(source, sourcePath, budget = createGraphBudget()) {
    ...
    traverse(ast, {
      enter(path) {
        ...
        if (path.isForInStatement?.() || path.isForOfStatement?.()) {
          bindingLoopWritePaths.push(path)
        }
  ...
  // Babel's binding crawl does not see every assignment through transparent
  // expression wrappers when createParenthesizedExpressions is enabled. Repair
  // the shared binding metadata before any context, export or callable proof.
  for (const path of activationAssignmentPaths) {
    registerTargetWrites(path.get(path.isUpdateExpression?.() ? 'argument' : 'left'), path)
  }
  for (const path of bindingLoopWritePaths) {
    const left = path.get('left')
    if (!left.isVariableDeclaration?.()) registerTargetWrites(left, path)
  }
  ...
  for (const path of implicitArgumentsPaths) {
    let owner = executionFunctionParent(path)
    while (owner?.isArrowFunctionExpression?.()) owner = executionFunctionParent(owner)
    const record = owner ? recordByNode.get(owner.node) : undefined
  ...
  const { context, forwarding } = contextReferenceResult(
    functionRecords,
    valueContextPaths,
    candidateByBinding,
    incompleteBindings,
  ...
  const functionSummaries = functionRecords.map((record) => {
    ...
    return {
      ...
      // Named default function exports can still have a reassigned live binding.
      callableStable: !record.path.isFunctionDeclaration?.() || !record.path.node.id
        || record.path.parentPath.scope.getBinding(record.path.node.id.name)?.constant === true,
  ...
  if (edgeCollector.overflow || callCollector.overflow) {
...
export function inspectExecutableModuleClosure(files, options = {}) {
  ...
  const modules = [...graph.reachable.values()].map((analysis) => {
    const forwarding = analysis.contextForwarding
    if (analysis.context.complete || !analysis.moduleClosureComplete
        || !forwarding?.localComplete || forwarding.calls.length === 0) return analysis
    // Transfer context coverage only to an inspected ctx binding. Incomplete
    // uses remain on the target module; every reachable module is checked.
    // Opaque code in either module can mutate a live callable binding through
    // effects outside the bounded activation model; retain refusal in that case.
    // Source-only metadata and route proof retain their conservative result.
    const complete = forwarding.calls.every((descriptor) => {
      const target = resolveCallableDescriptor(graph, files, analysis.sourcePath, descriptor)
      return target.ok && !target.terminal && target.summary.candidate
        && graph.reachable.get(target.sourcePath)?.moduleClosureComplete === true
        && target.summary.usable && target.summary.mutationComplete && target.summary.callableStable
    })
    return complete ? { ...analysis, context: { ...analysis.context, complete: true } } : analysis
  })
