...
import { agentProjectRoot, createNativeToolDefinition, NATIVE_TOOL_NAME, resolveNativeSource } from './native-tool-internal.js'
...
async function executeOperation({ authoritySources, tools }, input, options) {
  ...
  if (input.operation === 'project') {
    ...
    report = await inspectSession(input.source, { sourceRoot: agentProjectRoot(options.agent), limit: input.limit, signal: options.signal })
  } else if (input.operation === 'knowledge') {
    const project = input.source === undefined ? undefined : await inspectProject(input.source, {
      sourceRoot: agentProjectRoot(options.agent), signal: options.signal,
    })
    report = await inspectDshKnowledge({ dshPath: runningDshEntry(), upstreamRoot: project?.project.root,
      topic: input.topic, packageName: input.packageName, signal: options.signal })
