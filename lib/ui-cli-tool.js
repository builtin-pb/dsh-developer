...
/** @import { UiController, UiDependencies, UiContext, ToolLookupSource } from './native-tool-types.js' */
...
/** @param {UiController} controller @returns {ToolDefinition} */
export function createUiCliToolDefinition(controller) {
  ...
  /** @type {WeakMap<Agent, { ended: boolean }>} */
  ...
  return {
    ...
    output: {
      ...
      render(_args, value) {
        /** @type {unknown} */
        const text = formatUiCliReport(value)
        if (typeof text !== 'string') throw new TypeError('Invalid UI report rendering')
    ...
    async execute(args, exec) {
      ...
      if (!exec.agent || typeof sessionId !== 'string' || sessionId.length === 0 || typeof exec.agent.ctx?.effect !== 'function') {
...
export async function registerUiCliToolWithDependencies({ tools, effect, onConfigurationError }) {
  ...
  try {
    ...
    if (!(error instanceof DshDeveloperError) || typeof error.code !== 'string' || !CONFIGURATION_ERRORS.has(error.code) || !onConfigurationError) throw error
