...
async function scanTreeOnce(root, signal, excludeDependencies) {
  const entries = []
...
export async function scanOrdinaryTree(root, options = {}) {
  ...
  const first = await scanTreeOnce(absoluteRoot, options.signal, excludeDependencies)
  const second = await scanTreeOnce(absoluteRoot, options.signal, excludeDependencies)
