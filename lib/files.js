...
async function scanTreeOnce(root, signal, excludeDependencies, allowBinaryAssets) {
  const entries = []
  ...
  async function visit(directory) {
    ...
    for (const child of children) {
      ...
      entries.push({
        ...
        snapshotBytes: binary ? buffer : content,
      })
    }
  }
  ...
  for (const entry of entries) {
    ...
    hash.update('\0')
    hash.update(entry.snapshotBytes, 'utf8')
    hash.update('\0')
...
export async function scanOrdinaryTree(root, options = {}) {
  ...
  const first = await scanTreeOnce(absoluteRoot, options.signal, excludeDependencies, allowBinaryAssets)
  const second = await scanTreeOnce(absoluteRoot, options.signal, excludeDependencies, allowBinaryAssets)
