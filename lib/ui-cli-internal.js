...
// @playwright/cli 0.1.18 generates eN or fNeN refs (nonzero frame sequence).
// Keep both numeric components bounded; never admit the provider's selector fallback.
const TARGET_PATTERN = /^(?:f[1-9][0-9]{0,5})?e[1-9][0-9]{0,5}$/u
...
export function parseUiCliInput(value) {
  ...
  if (value.target !== undefined) {
    ...
    if (!TARGET_PATTERN.test(target)) inputError('target must be an exact element ref from snapshot or find, such as e12 or f1e140.')
    input.target = target
  }
...
async function safeDirectory(path) {
  try {
    ...
    if (['EACCES', 'EPERM', 'EROFS', 'EEXIST', 'ENOTDIR', 'ENOENT', 'ELOOP'].includes(error.code)) {
      throw new DshDeveloperError('UI_ROOT_INVALID', 'The UI CLI storage directory is unavailable; check its location and permissions.')
    }
    throw error
  }
}
...
export class UiCliController {
  ...
  async execute(sessionId, value, options = {}) {
    ...
    let owner = this.#owners.get(identity.internal)
    if (!owner) {
      owner = { identity, controller: new AbortController(), disposal: undefined }
      this.#owners.set(identity.internal, owner)
    }
    if (owner.controller.signal.aborted) throw new DshDeveloperError('UI_OWNER_DISPOSED', 'This UI owner has ended.')
    const signal = options.signal ? AbortSignal.any([options.signal, owner.controller.signal]) : owner.controller.signal
    const prior = this.#queues.get(identity.internal) ?? Promise.resolve()
    const operation = prior.catch(() => {}).then(async () => {
      // A cancelled queued call must never reach the provider, even if the
      // previous call swallowed cancellation while finishing its own cleanup.
      if (signal.aborted) throw new DshDeveloperError('CANCELLED', 'The UI operation was cancelled.')
      const report = await this.#executeUnlocked(identity, input, signal)
      if (signal.aborted) throw new DshDeveloperError('CANCELLED', 'The UI operation was cancelled.')
      return report
    })
  ...
  async #executeUnlocked(identity, input, signal) {
    ...
    let invoked
    // A provider may create its browser before open rejects or is cancelled.
    // Retain that possible ownership until a close command actually succeeds.
    if (input.operation === 'open') this.#active.set(identity.internal, identity)
    try {
  ...
  async #closeQuietly(identity) {
    try {
      ...
      this.#active.delete(identity.internal)
    } catch { /* Preserve the operation error and retain ownership for disposal. */ }
  }
  ...
  disposeOwner(sessionId) {
    if (this.#disposed) return this.dispose()
    const identity = uiCliSessionIdentity(sessionId)
    const owner = this.#owners.get(identity.internal)
    return owner ? this.#disposeOwner(owner) : Promise.resolve()
  }
  ...
  #disposeOwner(owner) {
    ...
    // Keep the closing owner until its queue drains. The native tool rejects
    // ended Agent objects; a later resumed Agent may reuse the same session ID.
    owner.disposal = (async () => {
      await this.#queues.get(identity.internal)?.catch(() => {})
      if (this.#active.has(identity.internal)) {
        await this.#invoke(identity, ['close'], { signal: AbortSignal.timeout(5_000), timeoutMs: 5_000 })
        this.#active.delete(identity.internal)
      }
      this.#owners.delete(identity.internal)
    })().catch(error => {
      // Remain ended and retain possible browser ownership. Only an explicit
      // cleanup retry may proceed; ordinary calls cannot reopen this owner.
  ...
  dispose() {
    ...
    this.#disposed = true
    // Abort all owners before awaiting any of them; one busy owner must not
    // delay cancellation of another owner's provider process.
    this.#disposal = Promise.allSettled([...this.#owners.values()].map(owner =>
      this.#disposeOwner(owner))).then(results => {
      const failures = results.filter(result => result.status === 'rejected').map(result => result.reason)
      if (failures.length) throw new AggregateError(failures, 'Could not close all UI sessions.')
    }).catch(error => {
