...
/** @param {unknown} value */
export function isUiLoopbackUrl(value) {
...
export function uiSafetyGuardReason(exec) {
  ...
  if (URL_OPERATIONS.has(rawName)) {
    const url = args !== null && typeof args === 'object' && 'url' in args ? args.url : undefined
    if (!isUiLoopbackUrl(url)) {
  ...
  if (rawName === 'browser_tabs'
      && args !== null
      && typeof args === 'object'
      && 'action' in args && args.action === 'new'
      && 'url' in args && args.url !== undefined
      && !isUiLoopbackUrl(args.url)) {
    return 'dsh-developer admits new dsh_ui tabs only at explicit HTTP(S) loopback URLs.'
  }
  return undefined
}

/** @param {import("@deepseek-ai/dsh-tools").ToolRuntime} tools */
export function registerUiSafetyGuardWithTools(tools) {
  return tools.guard(uiSafetyGuardReason)
}

/** @param {import("@deepseek-ai/cordis").Context} ctx */
export function registerUiSafetyGuard(ctx) {
  return registerUiSafetyGuardWithTools(ctx.tools)
}
