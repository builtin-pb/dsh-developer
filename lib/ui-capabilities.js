...
export function inspectUiCapabilities(value, options = {}) {
  ...
  if (selected === undefined) {
    checks.push(check(
      ...
      'FAIL',
      true,
      'No recognized semantic browser provider is visible. Run dsh-developer ui-setup, restart DSH, then rerun UI admission; check Agent tool scope if the provider remains absent.',
    ))
  } else {
    checks.push(check(
      'semantic-provider',
