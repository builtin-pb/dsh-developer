...
export async function createAppleContainerCell(options = {}) {
  ...
  try {
    ...
    try { await dispose() } catch (cleanup) {
      throw failure('CELL_CREATE_CLEANUP_FAILED', 'VM creation failed and cleanup could not be verified.', {
        providerId: provider.id, cellId: name, creation: asDiagnostic(error), cleanup: asDiagnostic(cleanup),
      })
    }
    throw error
  }
