...
const OPERATION_FIELDS = Object.freeze({
  ...
  knowledge: new Set(['source', 'topic', 'packageName']),
...
export function parseNativeToolInput(value) {
  ...
  for (const key of ['source', 'profile', 'releaseDsh', 'previewDsh', 'packageName']) {
  ...
  return {
    ...
    parameters: {
      ...
      properties: {
        ...
        source: { type: 'string', description: 'Project, plugin, Creator export, or workspace-confined hook/session file; knowledge uses an upstream checkout.' },
        ...
        topic: { type: 'string', enum: [...KNOWLEDGE_TOPICS], description: 'DSH development topic for knowledge; source optionally selects an upstream checkout inside this workspace.' },
        packageName: { type: 'string', description: 'Exact @deepseek-ai/name package to focus knowledge on; follow a declaration import by querying its owning package.' },
