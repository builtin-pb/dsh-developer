...
function requiredServices(references) {
  return references.services
    .filter((value) => value.evidence.some((item) => item.kind === 'inject'))
...
export async function inspectProfilePreflightInternal(source, options, dependencies = {}) {
  ...
  if (runtime) {
    try {
      ...
      const owners = await indexServices(installed, { signal: options.signal })
    ...
    checks.push(check(
      'profile.service-contract',
      'SKIP',
      'Profile service preflight requires an exact reviewed official DSH entry.',
      true,
    ))
  }
