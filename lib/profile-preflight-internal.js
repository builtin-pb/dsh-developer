...
import { scanSourceAuditTree } from './files.js'
...
export async function inspectProfilePreflightInternal(source, options, dependencies = {}) {
  ...
  const checks = [check(
    ...
    true,
    { fingerprint: first.fingerprint, files: first.fileCount, budget: first.budget },
  )]
  ...
  if (owners && injectContract.ok) {
    ...
    checks.push(check(
      'profile.service-contract',
      'SKIP',
      'Profile composition remains blocked. ' + describeHostAttachmentFailure(injectContract),
      true,
      injectContract,
    ))
  }
