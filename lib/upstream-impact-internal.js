...
export async function inspectUpstreamImpactInternal(source, options, dependencies = {}) {
  ...
  const first = await scan(resolve(source), { signal: options.signal, excludeDependencies: true, allowBinaryAssets: true })
  ...
  const final = await scan(resolve(source), { signal: options.signal, excludeDependencies: true, allowBinaryAssets: true })
