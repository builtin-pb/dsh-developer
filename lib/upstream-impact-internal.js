...
const SERVICE_NAME = /^[A-Za-z_$][A-Za-z0-9_$-]{0,127}$/u
...
export function discoverUpstreamReferences(tree) {
  ...
  for (const path of closure.runtime) {
    ...
    const injectDeclarations = [
      ...content.matchAll(/^\s*(?:export\s+)?const\s+inject\s*=\s*\[([\s\S]*?)\]/gmu),
      ...content.matchAll(/^\s*(?:export\s+)?const\s+inject\s*=\s*\{([\s\S]*?)\}/gmu),
    ]
    if (injectDeclarations.length === 0) continue
    for (const match of injectDeclarations) {
      for (const value of match[1].matchAll(/["']([A-Za-z_$][A-Za-z0-9_$-]{0,127})["']/gu)) {
        addServiceReference(services, value[1], { kind: 'inject', path })
      }
    }
