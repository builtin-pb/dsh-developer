...
function injectLiteralShapeComplete(body, kind) {
  let remainder = body.replace(/(["'])([^"'\\\r\n]*)\1/gu, '')
  if (kind === 'object') {
    remainder = remainder
...
function invalidInjectLiteralValues(body) {
  return [...body.matchAll(/(["'])([^"'\\\r\n]*)\1/gu)]
    .map((value) => value[2])
    .filter((value) => !SERVICE_NAME.test(value))
}
...
export function discoverUpstreamReferences(tree) {
  ...
  for (const path of closure.runtime) {
    ...
    for (const declaration of injectDeclarations) {
      const complete = injectLiteralShapeComplete(declaration.match[1], declaration.kind)
      if (!complete) unparsedInjectDeclarations.add(path)
      if (complete) {
        for (const value of invalidInjectLiteralValues(declaration.match[1])) {
          addInvalidInjectValue(invalidInjectValues, { path, kind: 'inject', value })
        }
      }
    ...
    const contextInjectStarts = [...content.matchAll(/\bctx\.inject\s*\(/gu)]
    const contextInjectDeclarations = [
      ...content.matchAll(/\bctx\.inject\(\s*\[([\s\S]*?)\]\s*,/gu),
    ]
    if (contextInjectStarts.length !== contextInjectDeclarations.length) {
      unparsedInjectDeclarations.add(path)
    }
    for (const declaration of contextInjectDeclarations) {
      if (!injectLiteralShapeComplete(declaration[1], 'array')) {
        unparsedInjectDeclarations.add(path)
        continue
      }
      for (const value of invalidInjectLiteralValues(declaration[1])) {
        addInvalidInjectValue(invalidInjectValues, { path, kind: 'context-inject', value })
      }
      for (const value of injectServiceReferences(declaration[1], 'array')) {
        addServiceReference(services, value.service, {
          kind: 'context-inject',
          requirement: 'runtime',
          path,
