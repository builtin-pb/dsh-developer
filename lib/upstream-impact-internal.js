...
import { mapTreeEntries, scanSourceAuditTree } from './files.js'
...
export function discoverUpstreamReferences(tree) {
  ...
  const incompleteContextReferences = new Set()
  ...
  for (const executableModule of closure.modules) {
    ...
    if (!contextReferences.complete) incompleteContextReferences.add(path)
  ...
  if (patchPath && files.has(patchPath)) {
    if (typeof files.get(patchPath) !== 'string') {
      throw new DshDeveloperError('BINARY_FILE', 'The DSH bundle patch must be UTF-8 text.', { path: patchPath })
    }
  ...
  return {
    ...
    coverage: {
      ...
      unparsedInjectDeclarations: [...unparsedInjectDeclarations]
        .sort((left, right) => left.localeCompare(right, 'en')),
      incompleteContextReferences: [...incompleteContextReferences]
        .sort((left, right) => left.localeCompare(right, 'en')),
      incompleteActivationClosure: [...closure.activationIncompletePaths],
...
export function classifyHostInjectContract(references) {
  ...
  const contextPaths = references.coverage.incompleteContextReferences ?? []
  const activationPaths = references.coverage.incompleteActivationClosure ?? []
  ...
  return {
    ...
    invalidValues,
    unparsedDeclarations,
    // Validity describes discovered declarations, not proof that none are hidden.
    injectionValidity: { ok: injectionPaths.length === 0, paths: injectionPaths },
    contextCoverage: { complete: contextPaths.length === 0, paths: contextPaths },
    activationCoverage: { complete: activationPaths.length === 0, paths: activationPaths },
  }
}

/** Explain the existing blocking decision without attributing every coverage gap to inject syntax. */
export function describeHostAttachmentFailure(contract) {
  const categories = [
    ['client-package-injection', contract.clientPackageInjections.map((value) => value.path)],
    ['invalid-service-name', contract.invalidValues.map((value) => value.path)],
    ['injection-declaration', contract.unparsedDeclarations],
    ['context-coverage', contract.contextCoverage.paths],
    ['activation-coverage', contract.activationCoverage.paths],
  ].filter(([, paths]) => paths.length > 0)
  const locations = categories.map(([category, paths]) => `${category}: ${[...new Set(paths)].join(', ')}`)
  return 'Upstream attachment proof is blocked (' + locations.join('; ') + '). '
    + (contract.clientPackageInjections.length > 0
      ? 'Host inject accepts service names; browser package identifiers belong in package.json dsh.client.inject. ' : '')
    + (contract.contextCoverage.paths.length > 0
      ? 'To delegate ctx, call a stable local/imported helper directly as register(ctx), with ctx as the sole argument. '
        + 'Its first parameter must be ctx and its body must pass the same checks; caller and helper modules must have no opaque runtime code. You can also export { register as apply }. '
        + 'For other helper signatures, keep service lookups and registrations in an inspected ctx function and pass resolved services or data. '
        + 'Adding inject entries does not prove context coverage. ' : '')
...
export async function inspectUpstreamImpactInternal(source, options, dependencies = {}) {
  const scan = dependencies.scanSourceAuditTree ?? dependencies.scanOrdinaryTree ?? scanSourceAuditTree
  ...
  const checks = [
    check('source.snapshot', 'PASS', 'Acquired one stable, bounded plugin tree with no detected credential markers.', true, {
  ...
  checks.push(check(
    ...
    injectContract.ok
      ? 'Discovered inject declarations are valid and context/activation coverage is complete.'
      : describeHostAttachmentFailure(injectContract),
    true,
    injectContract.ok ? undefined : injectContract,
  ))
  ...
  checks.push(check(
    ...
    true,
    { before: first.fingerprint, after: final.fingerprint, budget: final.budget },
  ))
