...
const STAGE_OPTIONS = new Set(['signal'])
  ...
  const handle = {
    ...
    async stageResult(stageOptions = {}) {
      ...
      const operation = (async () => {
        ...
        const stagingRoot = await makeTemp(join(stagingBase, '.dsh-developer-cell-result-'))
        const destination = join(stagingRoot, 'result')
        try {
          ...
          await makeDirectory(destination, { mode: 0o700 })
