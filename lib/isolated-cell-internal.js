...
import { createAppleContainerCell } from './lab/apple-container.js'
...
export async function openAdmittedIsolatedCellInternal(source, options, dependencies = {}) {
  const lease = acquireCellAdmissionLease(options.admission)
  const openCell = dependencies.openCell ?? openIsolatedCellInternal
  try {
    return await openCell(source,
      { signal: options.signal, distro: lease.grant.distro, providerId: lease.grant.providerId },
      { onDisposed: lease.release })
  } catch (error) {
    // The provider has no published handle on creation failure. A cleanup
    // failure therefore retains the lease until external recovery is proved;
    // absence of a handle is not proof that its owned environment is gone.
    if (error?.code !== 'CELL_CREATE_CLEANUP_FAILED') lease.release()
    throw error
  }
}
...
export async function openIsolatedCellInternal(source, options = {}, dependencies = {}) {
  ...
  const createCell = dependencies.createWslBubblewrapCell
    ?? (options.providerId === 'apple-container' ? createAppleContainerCell : createWslBubblewrapCell)
  ...
  const handle = {
    ...
    provider: {
      id: cell.provider.id,
      ...(cell.provider.distro === undefined ? {} : { distro: cell.provider.distro }),
      ...(cell.provider.kernel === undefined ? {} : { kernel: cell.provider.kernel }),
      ...(cell.provider.bwrapVersion === undefined ? {} : { bwrapVersion: cell.provider.bwrapVersion }),
    },
    ...
    async dispose() {
      ...
      if (disposePromise === undefined) {
        ...
        disposePromise = (async () => {
          const providerDisposal = Promise.resolve().then(() => cell.dispose())
          // Observe removal failure immediately, but settle the active operation
          // before reporting disposal or releasing its admission lease.
          const [, disposal] = await Promise.allSettled([pending, providerDisposal])
