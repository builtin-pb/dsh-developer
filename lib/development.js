...
/** DSH's Windows plugin forwarder crosses a second, cmd.exe shell boundary. */
export function developmentPathArgument(path, platform = process.platform) {
...
async function prepare(source, options) {
  ...
  const sourceArgument = developmentPathArgument(selected)
  const info = await lstat(selected)
  ...
  try {
    ...
    await runDsh(invocation, ['plugin', '--profile', profile, 'add', sourceArgument, '--ignore-scripts',
      '--store-dir', developmentPathArgument(join(home, 'pnpm-store')), ...(options.online ? [] : ['--offline'])], {
