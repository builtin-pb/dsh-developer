...
export function validateToolCases(value) {
  ...
  for (const item of value) {
    if (!item || typeof item !== 'object' || Array.isArray(item)
        || Object.keys(item).some(key => !['name', 'tool', 'arguments', 'expected', 'isError', 'errorContains', 'resultPath', 'maxResultBytes'].includes(key))
    ...
    if (item.errorContains !== undefined && (item.isError !== true || typeof item.errorContains !== 'string'
        || !item.errorContains.trim() || item.errorContains.length > 512)) {
      throw new DshDeveloperError('DEVELOPMENT_CASES_INVALID', 'errorContains requires isError: true and a nonblank literal string of at most 512 characters.')
    }
  }
  return value
}
  ...
  try {
    ...
    if (stat?.isFile() && !stat.isSymbolicLink() && stat.size <= MAX_CASE_BYTES) {
      ...
      if (Buffer.byteLength(text) <= MAX_CASE_BYTES) try {
        ...
        const processProtection = executionOutput?.incomplete ? 'incomplete-process-output'
          : executionOutput?.withheld?.stdout || executionOutput?.withheld?.stderr ? 'private-key' : undefined
