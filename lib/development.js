...
import { protectVerificationReceipt, validVerificationReceipt } from './development-receipt.js'
...
function hasUnsupportedJsonNumber(value) {
  const pending = [value]
  const seen = new WeakSet()
  while (pending.length) {
    const current = pending.pop()
    if (typeof current === 'number' && (!Number.isFinite(current) || Object.is(current, -0))) return true
...
export function validateToolCases(value) {
  ...
  const fields = ['name', 'tool', 'arguments', 'expected', 'isError', 'errorContains', 'resultPath', 'maxResultBytes']
  const invalid = (index, field, reason) => {
    throw new DshDeveloperError('DEVELOPMENT_CASES_INVALID', `Case #${index + 1}: ${field} ${reason}`,
      { caseIndex: index + 1, field })
  }
  // Check cases and fields in a fixed order, independent of JSON key order.
  // Only fixed schema labels and reasons enter diagnostics, never user data
  // (including case names, tool names and unknown property names).
  for (const [index, item] of value.entries()) {
    if (!item || typeof item !== 'object' || Array.isArray(item)) {
      invalid(index, 'case', 'must be an object.')
    }
    if (Object.keys(item).some(key => !fields.includes(key))) {
      invalid(index, 'fields', 'contain an unsupported property; allowed fields: ' + fields.join(', ') + '.')
    }
    if (item.name !== undefined) {
      if (typeof item.name !== 'string' || !item.name.trim()) invalid(index, 'name', 'must be a nonblank string.')
      if (item.name.length > 128) invalid(index, 'name', 'must be at most 128 characters.')
      if (/[\u0000-\u001f\u007f-\u009f]/u.test(item.name)) invalid(index, 'name', 'must not contain control characters.')
    }
    if (typeof item.tool !== 'string' || !/^[a-zA-Z0-9][a-zA-Z0-9_.:-]{0,127}$/u.test(item.tool)) {
      invalid(index, 'tool', 'must be 1–128 characters, start with an ASCII letter or digit, and contain only ASCII letters, digits, _, ., : or -.')
    }
    if (!item.arguments || typeof item.arguments !== 'object' || Array.isArray(item.arguments)) {
      invalid(index, 'arguments', 'must be an object.')
    }
    for (const field of ['arguments', 'expected']) {
      if (hasUnsupportedJsonNumber(item[field])) {
        invalid(index, field, 'must not contain non-finite numbers or negative zero; JSON transport would change them.')
      }
    }
    if (item.isError !== undefined && typeof item.isError !== 'boolean') {
      invalid(index, 'isError', 'must be boolean.')
    }
    if (item.maxResultBytes !== undefined && (!Number.isSafeInteger(item.maxResultBytes) || item.maxResultBytes < 1)) {
      invalid(index, 'maxResultBytes', 'must be a positive safe integer.')
    }
    if (item.resultPath !== undefined) {
      if (typeof item.resultPath !== 'string') invalid(index, 'resultPath', 'must be a string.')
      if (item.resultPath.length > 512) invalid(index, 'resultPath', 'must be at most 512 characters.')
      if (!/^(?:\/(?:[^~]|~[01])*)?$/u.test(item.resultPath)) {
        invalid(index, 'resultPath', 'must be a JSON Pointer: empty or starting with /, with ~ escaped as ~0 and / within a key as ~1.')
      }
    }
    if (!Object.hasOwn(item, 'expected') && item.isError !== true) {
      invalid(index, 'expected', 'is required for a successful case; use isError: true for an expected failure.')
    }
    if (item.errorContains !== undefined) {
      if (item.isError !== true) invalid(index, 'errorContains', 'requires isError: true.')
      if (typeof item.errorContains !== 'string' || !item.errorContains.trim()) {
        invalid(index, 'errorContains', 'must be a nonblank literal string.')
      }
      if (item.errorContains.length > 512) invalid(index, 'errorContains', 'must be at most 512 characters.')
    }
  }
  return value
}

export { validVerificationReceipt } from './development-receipt.js'

async function developmentWorkspace(path) {
  ...
  throw new DshDeveloperError('DEVELOPMENT_WORKSPACE_INVALID', '--workspace must select an existing directory.')
}
...
/** Only the legacy pnpm forwarder needs caller-supplied Windows shell quotes. */
export async function developmentInstallerNeedsQuotes(installed, platform = process.platform) {
  if (platform !== 'win32') return false
  const entry = pathToFileURL(resolve(installed.root, installed.value.bin.dsh))
  const cli = await readFile(entry, 'utf8')
  // Follow the selected CLI's actual chunk, not a version or the presence of
  // dsh-plugin-manager elsewhere in the dependency graph. Newer forwarders use
  // Execa, which owns Windows escaping and must receive unquoted arguments.
  const plugin = cli.match(/\bimport\s*\(\s*(['"])(\.\/plugin(?:-[\w-]+)?\.js)\1\s*\)/u)
  const forwarder = plugin ? await readFile(new URL(plugin[2], entry), 'utf8') : cli
  return /\bspawnSync\s*\(\s*(['"])pnpm\1\s*,/u.test(forwarder)
    && /\bshell\s*:\s*process\.platform\s*===\s*(['"])win32\1/u.test(forwarder)
}

/** Quote only for an observed legacy cmd.exe boundary, never for direct argv. */
export function developmentPathArgument(path, platform = process.platform, needsQuotes = true) {
  if (platform !== 'win32' || !needsQuotes) return path
  if (/["%!\u0000-\u001f]/u.test(path)) {
    throw new DshDeveloperError('DEVELOPMENT_PATH_UNSUPPORTED',
      'DSH\'s Windows installer cannot safely forward this source or temporary path. Use a path without percent signs, exclamation marks, double quotes or control characters.')
  }
  // The official CLI forwards these quote characters to its native pnpm
  // shell command. They are not quoting for our shell:false Node launch.
  return '"' + path + '"'
}
...
async function prepare(source, options) {
  ...
  if (options.watch && !info.isDirectory()) {
    throw new DshDeveloperError('DEVELOPMENT_WATCH_SOURCE_INVALID', 'Hot reload needs a plugin source directory, not a packed archive.')
  }
  ...
  const needsQuotes = await developmentInstallerNeedsQuotes(installed)
  const sourceArgument = developmentPathArgument(selected, process.platform, needsQuotes)
  ...
  try {
    ...
    await runDsh(invocation, ['plugin', '--profile', profile, 'add', sourceArgument, '--ignore-scripts',
      '--store-dir', developmentPathArgument(join(home, 'pnpm-store'), process.platform, needsQuotes), ...(options.online ? [] : ['--offline'])], {
...
async function notifyDevelopment(callback, report, signal) {
  ...
  try {
    // Cancellation owns the server lifecycle, even when a consumer's callback
    // never settles. Promise.race still consumes a later callback rejection.
    await Promise.race([Promise.resolve().then(() => signal.aborted ? undefined : callback(report)), cancelled])
  } catch (error) {
    throw error instanceof Error ? error : new DshDeveloperError('DEVELOPMENT_CALLBACK_FAILED', 'Development status callback rejected.')
...
export async function verifyDevelopmentPlugin(source, options = {}) {
  const workspace = await developmentWorkspace(options.workspacePath)
  ...
  try {
    ...
    try {
      ...
      const execution = await runDsh(invocation, ['--profile', state.profile, ...state.patchArgs, '--patch', patchPath, ...appArgs], {
      ...
      executionOutput = error.details?.output
    ...
    let result, receiptPrivateKey = false
    if (stat?.isFile() && !stat.isSymbolicLink() && stat.size <= MAX_CASE_BYTES) {
      const text = await readFile(resultPath, 'utf8')
      if (Buffer.byteLength(text) <= MAX_CASE_BYTES) try {
        receiptPrivateKey = findSecrets(text).includes('private-key')
        const parsed = JSON.parse(text)
        receiptPrivateKey ||= parsed?.privateKeyOutput === true
          || (Array.isArray(parsed?.cases) && parsed.cases.some(item => item?.privateKeyOutput === true))
        let processProtection
        if (executionOutput?.incomplete) processProtection = 'incomplete-process-output'
        else if (executionOutput?.withheld?.stdout || executionOutput?.withheld?.stderr) processProtection = 'private-key'
        if (validVerificationReceipt(parsed, cases, workspace)) result = protectVerificationReceipt(parsed, cases, processProtection)
      } catch { /* Missing evidence is a failure below. */ }
    }
    if (executionError && (receiptPrivateKey || result?.outputProtection?.reason === 'private-key')) {
      // A marker may be in the receipt while its body was logged by DSH. Keep
      // process facts, never diagnostic fragments from the other channel.
      const { code, exitCode, exitSignal, output } = executionError
      executionError = { code, ...(exitCode === undefined ? {} : { exitCode }),
        ...(exitSignal === undefined ? {} : { exitSignal }),
        ...(output === undefined ? {} : { output: { ...output, withheld: { stdout: true, stderr: true } } }),
        message: 'Development process diagnostics withheld because this invocation contained private-key material.' }
    }
    ...
    const report = { kind: 'dsh-development-verification', ok, complete, caseCount: cases.length,
      ...
      ...(result?.agent ? { agent: { ...result.agent, workspace } } : {}),
      ...
      ...(result?.outputProtection ? { outputProtection: result.outputProtection } : {}),
...
export async function runDevelopmentServer(source, options = {}) {
  const selectedWorkspace = await developmentWorkspace(options.workspacePath)
  if (options.watch !== undefined && typeof options.watch !== 'boolean') {
    throw new DshDeveloperError('DEVELOPMENT_WATCH_INVALID', 'watch (--watch) must be boolean.')
  }
  ...
  let outcome, running, url, server, closeHandoff, exited, reloadTask, reloadError
  try {
    const workspace = selectedWorkspace ?? state.project?.root ?? join(state.home, 'workspace')
    if (!selectedWorkspace && !state.project) await mkdir(workspace)
    const reloadPatch = options.watch ? '- id: hmr\n  disabled: false\n  config:\n    base: '
      + JSON.stringify(pathToFileURL(state.source + sep).href) + '\n    root: ["."]\n' : ''
    await writeFile(patchPath, BACKGROUND_WEB_PATCH + reloadPatch
      + '- insert:\n    - id: dsh-developer-server-observer\n      name: '
      + JSON.stringify(new URL('./development-server-probe.js', import.meta.url).href) + '\n'
      + (options.watch ? '      config:\n        watch: true\n' : ''), { flag: 'wx' })
    // Native Cordis HMR owns graph invalidation, plugin disposal and reload.
    // Enable its required Node loader surface only for this opt-in process.
    const invocation = options.watch ? { ...state.invocation, prefixArgs: ['--expose-internals', ...state.invocation.prefixArgs] } : state.invocation
    running = runDsh(invocation, ['--profile', 'web', ...state.patchArgs, '--patch', patchPath, '--port', String(options.port ?? 0), '--no-open'], {
      cwd: workspace, env: { ...state.environment, DSH_DEVELOPER_SERVER_TOKEN: token,
        ...
        if (!url) {
          ...
          if (Object.hasOwn(server, 'error')) {
            if (server.kind !== 'dsh-development-server-private' || server.version !== 1
                ...
                || (server.privateKeyOutput !== undefined && typeof server.privateKeyOutput !== 'boolean')) {
            ...
            throw new DshDeveloperError('DEVELOPMENT_SERVER_STARTUP_FAILED', redactSensitiveOutput(server.error),
              server.privateKeyOutput || findSecrets(server.error).includes('private-key') ? { privateKeyOutput: true } : {})
          }
    ...
    if (options.watch && options.onReload) {
      reloadTask = (async () => {
        let sequence = 0
        while (!signal.aborted && !outcome && !exited) {
          let observation
          try {
            const info = await lstat(resultPath + '.reload')
            if (info.isFile() && !info.isSymbolicLink() && info.size <= 2048) {
              const text = await readFile(resultPath + '.reload', 'utf8')
              if (Buffer.byteLength(text) <= 2048) {
                const receipt = JSON.parse(text)
                if (receipt.token === token && validReloadObservation(receipt) && receipt.sequence > sequence) {
                  const { sequence: next, attempt, warnings, status, active, inactive } = receipt
                  observation = { kind: 'dsh-development-reload', sequence: next, attempt, warnings, status, active, inactive }
                }
              }
            }
          } catch { /* A missing or incomplete observation is not a reload result. */ }
          if (observation && !signal.aborted) {
            sequence = observation.sequence
            await notifyDevelopment(options.onReload, observation, signal)
          }
          await new Promise(resolve => setTimeout(resolve, 100))
        }
      })().catch(error => { reloadError = error; controller.abort() })
    }
    await notifyDevelopment(options.onReady, { kind: 'dsh-development-server', ok: true, url: browserUrl, source: state.source,
      runtime: state.runtime, profile: 'web', home: state.home, pid: server.pid, pidOwnership: 'calling terminal',
      workspace: server.workspace,
      ...(options.watch ? { reload: { mode: 'native-host-hmr', root: state.source } } : {}),
      ui: { operation: 'open', developmentServer: state.home },
      note: 'HTTP readiness only; inspect the rendered plugin through the browser before claiming UI success.' }, signal)
    await running
    await reloadTask
    ...
    const withheld = error.details?.privateKeyOutput || details.output?.incomplete
      || details.output?.withheld?.stdout || details.output?.withheld?.stderr
    if (withheld) {
      error.message = 'Web development failed; diagnostics withheld because captured output could not be safely disclosed.'
      if (!details.output?.withheld?.stdout || !details.output?.withheld?.stderr) {
        details.stdout = details.stderr = '[redacted: development diagnostics withheld]\n'
      }
      details.output = { ...details.output, withheld: { stdout: true, stderr: true } }
    }
    if (options.signal?.aborted) {
      throw new DshDeveloperError('CANCELLED', 'Development server was cancelled.', details)
    }
    if (!withheld && (error.code === 'DEVELOPMENT_SERVER_TIMEOUT' || error.code === 'DEVELOPMENT_SERVER_EXITED')) {
...
export function validReloadObservation(value) {
  return value && ['pending', 'settled', 'warning', 'failed'].includes(value.status)
    && ['sequence', 'attempt', 'warnings', 'active', 'inactive'].every(key => Number.isSafeInteger(value[key]) && value[key] >= 0)
    && value.sequence > 0 && (value.status !== 'settled' || value.inactive === 0)
}

/** Human diagnostics keep process evidence useful without exposing omitted output. */
export function formatDevelopmentDiagnostic(diagnostic) {
  const output = diagnostic.output
  const containsKey = diagnostic.privateKeyOutput === true || findSecrets([diagnostic.message, diagnostic.stdout, diagnostic.stderr]
    .filter(value => typeof value === 'string').join('\n')).includes('private-key')
  const withheld = ['stderr', 'stdout'].filter(stream => containsKey || output?.incomplete || output?.withheld?.[stream])
  const message = withheld.length ? 'Development failed; diagnostics withheld because captured output could not be safely disclosed.'
    : redactSensitiveOutput(diagnostic.message)
  const lines = [diagnostic.code + ': ' + message]
  if (withheld.length) lines.push('Process output withheld: ' + withheld.join(', ') + '.')
  if (output?.incomplete) lines.push('Process output did not finish draining; descendant cleanup is unverified.')
  for (const stream of ['stderr', 'stdout']) {
    if (withheld.includes(stream) || typeof diagnostic[stream] !== 'string' || !diagnostic[stream].trim()) continue
    // Protect complete captured lines before shortening the display. Keep the
    // error header even when a long stack would push it out of the final tail.
    const protectedText = redactSensitiveOutput(diagnostic[stream]).trim()
    const rows = protectedText.split(/\r?\n/u)
    const errorRow = rows.findLastIndex(row => /\b(?:\w*Error(?:\s*\[[^\]]+\])?:|dsh:)/u.test(row))
    const start = errorRow < 0 ? Math.max(0, rows.length - 12) : errorRow
    const selected = rows.slice(start, start + 12).join('\n')
    const excerpt = selected.slice(0, 3000)
    const clipped = start > 0 || start + 12 < rows.length || selected.length > 3000 || output?.truncated?.[stream]
    lines.push('Process ' + stream + (clipped ? ' (excerpt; additional output omitted)' : '') + ':')
    lines.push(excerpt)
  }
  return lines.join('\n')
}
...
export function formatDevelopmentReport(report) {
  if (report.kind === 'dsh-development-reload') {
    const status = { pending: 'reload in progress', settled: 'enabled entries active',
      warning: 'reload warning; current code may be stale', failed: 'reload activation could not be confirmed' }[report.status]
    return `Host reload #${report.attempt}: ${status}; ${report.active} active, ${report.inactive} inactive; ${report.warnings} warnings total.`
  }
  ...
  else if (typeof report.workspace?.path === 'string') lines.push('Web workspace: ' + report.workspace.path)
  ...
  if (report.reload) lines.push('Native Host hot reload: ' + report.reload.root)
  ...
  for (const item of report.cases ?? []) {
    ...
    const detail = item.outputLimitExceeded ? 'output budget exceeded'
      : item.valueWithheld ? 'canonical value withheld (possible credentials)'
    ...
    const reasons = {
      ...
      'error-message-mismatch': 'Error text does not contain the requested literal.',
    ...
    if (Object.hasOwn(item, 'errorContains')) lines.push('  errorContains: ' + JSON.stringify(item.errorContains))
    if (item.errorContainsOmitted) lines.push('  errorContains: omitted from report; compared in full')
    const withheldFields = ['value', 'content', 'expected', 'resultPath', 'errorContains'].filter(key => item[key + 'Withheld'])
    if (withheldFields.length) lines.push('  Withheld possible credentials in: ' + withheldFields.join(', ') + '.')
  }
  ...
  if (report.outputProtection?.withheld) lines.push('Some verification output was withheld because it may contain credentials; case outcomes are retained.')
  if (report.diagnostic) lines.push(report.complete === false
    ? formatDevelopmentDiagnostic(report.diagnostic)
