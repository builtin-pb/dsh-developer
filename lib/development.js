...
export function validateToolCases(value) {
  ...
  for (const item of value) {
    if (!item || typeof item !== 'object' || Array.isArray(item)
        || Object.keys(item).some(key => !['tool', 'arguments', 'expected', 'isError', 'resultPath', 'maxResultBytes'].includes(key))
...
async function prepare(source, options) {
  ...
  try {
    ...
    await runDsh(invocation, ['plugin', '--profile', profile, 'add', selected, '--ignore-scripts', ...(options.online ? [] : ['--offline'])], {
      cwd: home, env: environment, signal: options.signal, timeoutMs: options.timeoutMs ?? 120_000, diagnosticOutput: true,
    })
    await runDsh(invocation, ['--profile', profile, ...state.patchArgs, '--dump-config'], {
      cwd: home, env: environment, signal: options.signal, diagnosticOutput: true,
...
export async function verifyDevelopmentPlugin(source, options = {}) {
  ...
  try {
    ...
    await writeFile(patchPath, disabledApplicationEntries.map(id => '- id: ' + id + '\n  disabled: true\n').join('')
      + '- insert:\n    - id: dsh-developer-verification\n      name: '
      + JSON.stringify(fileURLToPath(new URL('./development-probe.js', import.meta.url))) + '\n', { flag: 'wx' })
    try {
      await runDsh(state.invocation, ['--profile', state.profile, ...state.patchArgs, '--patch', patchPath, ...appArgs], {
        cwd: state.home, signal: options.signal, timeoutMs: options.timeoutMs ?? 60_000, diagnosticOutput: true,
    ...
    if (stat?.isFile() && !stat.isSymbolicLink() && stat.size <= MAX_CASE_BYTES) {
      ...
      if (!findSecrets(text).length) {
        try { result = JSON.parse(text) } catch { /* Missing evidence is a failure below. */ }
      }
    }
    const ok = !executionError && result?.ok === true && result.cases?.length === cases.length
    return { kind: 'dsh-development-verification', ok, source: state.source, runtime: state.runtime,
      ...
      ...(executionError || result?.error ? { diagnostic: executionError ?? { code: 'DEVELOPMENT_FAILED', message: result.error } } : {}),
...
export async function runDevelopmentServer(source, options = {}) {
  ...
  try {
    await writeFile(patchPath, '- id: web-runtime\n  config:\n    printUrl: false\n'
      ...
      + JSON.stringify(fileURLToPath(new URL('./development-server-probe.js', import.meta.url))) + '\n', { flag: 'wx' })
    running = runDsh(state.invocation, ['--profile', 'web', ...state.patchArgs, '--patch', patchPath, '--port', String(options.port ?? 0), '--no-open'], {
      cwd: state.project?.root ?? state.home, env: { ...state.environment, DSH_DEVELOPER_SERVER_TOKEN: token,
        DSH_DEVELOPER_SERVER_RESULT: resultPath, DSH_DEVELOPER_WORKSPACE: state.project?.root ?? state.home },
      signal, timeoutMs: options.timeoutMs ?? 0, outputMode: 'tail', diagnosticOutput: true,
      onExit: status => {
        exited = status
        void closeHandoff?.().catch(() => {})
        // After readiness, the server's exit also ends its descendants. A child
    ...
    while (true) {
      ...
      try {
        if (!url) {
          ...
          server = JSON.parse(await readFile(resultPath, 'utf8'))
          if (server.token !== token || server.host !== '127.0.0.1' || !Number.isInteger(server.port)
      ...
      } catch { /* Wait for the owned process, bounded by startup and caller cancellation. */ }
...
export function formatDevelopmentReport(report) {
  ...
  for (const item of report.cases ?? []) {
    ...
    lines.push(`${item.passed ? 'PASS' : 'FAIL'} ${item.tool}${size}: ${detail}`)
  }
