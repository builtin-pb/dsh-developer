...
export function validVerificationReceipt(value, cases) {
  ...
  if (value.complete) {
    return value.cases.length === cases.length && value.activeCase === null && !Object.hasOwn(value, 'error')
    ...
    if (stat?.isFile() && !stat.isSymbolicLink() && stat.size <= MAX_CASE_BYTES) {
      ...
      if (!findSecrets(text).length) {
        try {
          ...
          if (validVerificationReceipt(parsed, cases)) result = parsed
...
export function formatDevelopmentReport(report) {
  ...
  if (report.invocationError) lines.push('Invocation: ' + report.invocationError)
