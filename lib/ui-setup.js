...
import {
  ...
  ordinaryUiFile, privateUiDirectory, readUiSettings, saveUiSettings, uiConfigPath, uiSetupCommandInstruction, validateUiEntry,
...
export async function installUiCli(configPath, { signal, runner = runBounded, executable = process.execPath } = {}) {
  ...
  if (!npm) throw new DshDeveloperError('UI_SETUP_NPM_MISSING', 'Install npm alongside the running Node, then retry. ' + uiSetupCommandInstruction(true))
  ...
  try {
    ...
    throw new DshDeveloperError('UI_SETUP_INSTALL_FAILED', 'Could not install @playwright/cli@0.1.18 into dedicated setup storage; configuration was not saved. Fresh provider storage was removed. Check npm/network availability, then retry. ' + uiSetupCommandInstruction(true))
  }
}
...
async function setupUiInternal(options = {}) {
  ...
  if (!browser) throw new DshDeveloperError('UI_BROWSER_INVALID', 'No installed Chrome or Edge was found in normal locations. Install one or append --browser-executable with its absolute executable path. ' + uiSetupCommandInstruction())
