...
import { DSH_COMPATIBILITY_TARGET, DSH_PREVIEW_TARGET, LIMITS } from './constants.js'
import { doctorSource, reportDigest } from './doctor.js'
import { DshDeveloperError } from './errors.js'
import { renameDirectoryExclusive, renameFileExclusive } from './exclusive-rename.js'
import {
...
import {
  createCellApprovalContext,
  createCellWorkspaceInspector,
  validateCellPlanDigest as validateDigest,
} from './native-cell-context.js'
...
const FIXED_POLICY = Object.freeze({
  version: 1,
  profile: PROFILE,
  dshLanes: Object.freeze([DSH_COMPATIBILITY_TARGET, DSH_PREVIEW_TARGET]),
  admission: Object.freeze({ provider: process.platform === 'darwin' ? 'apple-container' : 'wsl2-bubblewrap', exact: true }),
...
function safeDiagnosticDetails(details, depth, seen) {
  ...
  for (const key of ['expiresAt', 'phase', 'disposition', 'evidenceDigest', 'expected', 'actual', 'recoveryState', 'providerId', 'cellId']) {
...
async function inspectOrdinaryDirectoryPhysical(path, signal) {
  const before = await ordinaryPathSnapshot(path, signal)
  const physical = resolve(await realpath(resolve(path)))
  const after = await ordinaryPathSnapshot(path, signal)
  const physicalInfo = await lstat(physical, { bigint: true })
  return {
    physical,
    snapshot: after,
    stable: physicalInfo.isDirectory()
      && !physicalInfo.isSymbolicLink()
      && samePathSnapshot(before, after)
      && sameIdentity(after.at(-1), {
        dev: String(physicalInfo.dev),
        ino: String(physicalInfo.ino),
      }),
  }
}
...
async function inspectManagedTreeIdentity(root, tree, options = {}) {
  ...
  const directories = []
  const files = []
  const rootInspection = await inspectOrdinaryDirectoryPhysical(absoluteRoot, options.signal)
  const rootSnapshot = rootInspection.snapshot
  if (!rootInspection.stable) {
...
export const inspectLiveAgentWorkspace = createCellWorkspaceInspector({
  snapshotPath: ordinaryPathSnapshot,
  samePathSnapshot,
  digest,
  freeze: deepFreeze,
})
  ...
  for (const initial of candidates) {
    ...
    while (!visited.has(candidate)) {
      ...
      if (!pathInside(record.workspace.root, candidate)
        ...
        try {
          const inspection = await inspectOrdinaryDirectoryPhysical(candidate, signal)
          if (inspection.stable) return { base: candidate, snapshot: inspection.snapshot }
        } catch {
...
async function verifyStageCapability(record, stage, options = {}) {
  const anchorInspection = await inspectOrdinaryDirectoryPhysical(stage.anchor, options.signal)
  const ancestry = anchorInspection.snapshot
  if (!anchorInspection.stable
      || !prefixSnapshotMatches(stage.baseSnapshot, ancestry)
  ...
  const anchorPhysical = anchorInspection.physical
  const workspacePhysical = resolve(await realpath(record.workspace.root))
  const profile = await assertProfileFence(record, options.signal)
  if (pathInside(workspacePhysical, anchorPhysical)
      || pathInside(anchorPhysical, workspacePhysical)
  ...
  if (options.requireRoot !== false) {
    ...
    const rootInspection = await inspectOrdinaryDirectoryPhysical(stage.root, options.signal)
    if (!rootInspection.stable || !sameIdentity(rootIdentity, stage.rootIdentity)) {
  ...
  if (options.requireDestination !== false) {
    ...
    const destinationInspection = await inspectOrdinaryDirectoryPhysical(stage.destination, options.signal)
    if (!destinationInspection.stable || !sameIdentity(destinationIdentity, stage.destinationIdentity)) {
...
async function verifyTransactionCapability(record, transaction, dependencies, options = {}) {
  const rootInspection = await inspectOrdinaryDirectoryPhysical(transaction.root, options.signal)
  const ancestry = rootInspection.snapshot
  if (!rootInspection.stable
      || !prefixSnapshotMatches(transaction.baseSnapshot, ancestry)
  ...
  for (const key of ['backup', 'candidate', 'held', 'failed']) {
    ...
    const childInspection = await inspectOrdinaryDirectoryPhysical(expected, options.signal)
    if (!childInspection.stable || !sameIdentity(current, transaction.identities[key])) {
...
async function assertNoOrphanTransactions(workspaceRoot, activeTransactionRoot, signal) {
  ...
  for (const candidate of candidates) {
      ...
      try {
        ...
        if (marker?.kind === 'dsh-developer-cell-apply-recovery'
            && ((marker.version === 1 && typeof marker.source === 'string')
              || (marker.version === 2 && typeof marker.sourcePathDigest === 'string'
                && /^sha256:[a-f0-9]{64}$/u.test(marker.sourcePathDigest)))) {
          recovery = {
            sourcePathDigest: marker.version === 1 ? digest(resolve(marker.source)) : marker.sourcePathDigest,
            state,
          }
          break
        }
    ...
    if (recovery !== undefined && recovery.sourcePathDigest === digest(resolve(workspaceRoot))) {
...
async function writeTransactionMarker(record, transaction, state) {
  ...
  const payload = JSON.stringify({
    ...
    version: 2,
    state,
    // Recovery matches the selected source without copying its absolute path
    // into a content-scanned journal. Preserve legacy markers in the reader.
    sourcePathDigest: digest(resolve(record.workspace.root)),
    sourceIdentity: record.workspace.identityDigest,
    sourceFingerprint: record.sourceFingerprint,
...
async function mintTransactionAuthority(record, dependencies, capability, signal) {
  ...
  const baseInspection = await inspectOrdinaryDirectoryPhysical(base, signal)
  const baseSnapshot = baseInspection.snapshot
  if (!baseInspection.stable) {
...
async function ensureOrdinaryParents(root, relativeFile, created = undefined) {
  ...
  for (const segment of segments) {
    ...
    try {
      ...
      const inspection = await inspectOrdinaryDirectoryPhysical(current)
      if (!info.isDirectory() || info.isSymbolicLink() || !inspection.stable) {
...
function absentDirectories(sourceIdentity, stageIdentity) {
  const staged = new Set(stageIdentity.directories.map((entry) => entry.path))
  // Empty children must move first so their parents can also be held. Rollback
  // restores these same directory identities in the reverse order.
  return sourceIdentity.directories.map((entry) => entry.path)
...
async function commitCandidate(record, transaction, dependencies, signal) {
  ...
  for (const path of oldPaths) {
    ...
    await moveTransactionPathExclusive(transaction, dependencies.renameFileExclusive, sourcePath, heldPath)
  ...
  for (const path of absentDirectories(record.sourceTreeIdentity, record.stage.physicalIdentity)) {
    const sourcePath = join(source, ...path.split('/'))
    ...
    await moveTransactionPathExclusive(transaction, renameDirectoryExclusive, sourcePath, heldPath)
  ...
  for (const path of newPaths) {
    ...
    await moveTransactionPathExclusive(transaction, dependencies.renameFileExclusive, candidatePath, destination)
...
async function moveTransactionPathExclusive(transaction, move, source, destination) {
  try {
    await move(source, destination)
  } catch (cause) {
    // Even a refused move can mean a concurrent file occupies the destination.
    // Ordinary rollback restores a full original tree and must not erase it.
    transaction.recoveryRequired = true
    throw error('CELL_APPLY_RECOVERY_REQUIRED',
      'An exclusive transaction move failed or was not acknowledged; preserve source and transaction without rollback or replay.',
      { retainedRoot: transaction.root, operation: safeCellWorkflowDiagnostic(cause) })
  }
}
...
async function moveCurrentFileToFailed(record, transaction, dependencies, path, identity, label) {
  ...
  await moveTransactionPathExclusive(transaction, dependencies.renameFileExclusive, sourcePath, failedPath)
}
...
async function rollbackSource(record, transaction, dependencies, failure) {
  ...
  for (const held of [...transaction.heldDirectories].reverse()) {
    ...
    await moveTransactionPathExclusive(transaction, renameDirectoryExclusive,
      held.heldPath, join(record.workspace.root, ...held.path.split('/')))
  }
  for (const held of [...transaction.heldFiles].reverse()) {
    await ensureOrdinaryParents(record.workspace.root, held.path)
    await moveTransactionPathExclusive(transaction, dependencies.renameFileExclusive,
      held.heldPath, join(record.workspace.root, ...held.path.split('/')))
  }

  // Only undo controller-owned moves. Any remaining difference belongs to a
  // concurrent writer or is unexplained; restoring the snapshot would erase it.
  const current = await dependencies.scanTree(record.workspace.root, {})
...
async function cleanupTransaction(record, dependencies, signal) {
  assertRecoveryAllowsCleanup(record)
  const transaction = record.transaction
...
function assertRecoveryAllowsCleanup(record) {
  if (record.creationFailure !== undefined) {
    throw error('CELL_CREATE_CLEANUP_FAILED',
      'Cell creation cleanup remains unverified; no handle proves disposal and capacity remains poisoned.',
      { retainedRoot: record.creationFailure.retainedRoot, providerId: record.creationFailure.providerId,
        cellId: record.creationFailure.cellId, cleanup: record.creationFailure })
  }
  const transaction = record.transaction
  if (record.phase === 'rollback-failed' || transaction?.recoveryRequired
      || (transaction?.mutationStarted && !record.applyCommitted && !transaction.rollbackVerified)) {
    throw error('CELL_APPLY_RECOVERY_REQUIRED',
      'Unverified source mutation retains the transaction and blocks destructive cleanup.',
      { retainedRoot: transaction?.root ?? null })
  }
}
...
function reportFailure(record, failure, commands, sourceAfter, cellDisposed) {
  return {
    ...
    cleanup: { cellDisposed, capacityReleased: false, requiresCellDiscard: record.creationFailure === undefined },
  }
}
...
export function createNativeCellWorkflowController(options = {}) {
  const dependencies = {
    ...
    renameFileExclusive: options.renameFileExclusive ?? renameFileExclusive,
    randomUUID: options.randomUUID ?? randomUUID,
    dshPath: options.dshPath,
  }
  const slot = options.slot ?? PROCESS_SLOT
  const { prepareApproval, approvalGuard, consumeApproval, settleExecution, clearApprovals } = createCellApprovalContext({
    assertController,
    prepare: prepareCellApproval,
    diagnostic: safeCellWorkflowDiagnostic,
  })
  let disposed = false
  ...
  /**
   * @param {import('./native-cell-context-types.js').CellApprovalInput} parsed
   * @param {import('@deepseek-ai/dsh-tools').ToolExecution['agent']} owner
   * @param {import('@deepseek-ai/dsh-tools').ToolExecution['signal']} signal
   * @returns {Promise<import('./native-cell-context-types.js').PreparedCellApproval>}
   */
  async function prepareCellApproval(parsed, owner, signal) {
    const selected = currentFor(owner, parsed.planDigest)
    ...
    if (parsed.operation === 'cell-run') {
      ...
      const evidence = await collectApplyEvidence(record, signal)
      evidenceDigest = evidence.evidenceDigest
      reason = renderApplyApprovalReason(record, evidenceDigest)
    }
    return { kind: 'ask', reason, digest: record.digest, evidenceDigest }
  }
  ...
  async function run(input, call = {}) {
    ...
    const operation = (async () => {
      try {
        ...
        if (cause?.code === 'CELL_CREATE_CLEANUP_FAILED') {
          record.creationFailure = safeCellWorkflowDiagnostic(cause)
          record.phase = 'cleanup-failed'
        }
        failure = signal?.aborted && record.creationFailure === undefined
  ...
  async function apply(input, call = {}) {
    ...
    const operation = (async () => {
      ...
      try {
        ...
        // Preparation records ownership before it can return. A failed await
        // must still clean or truthfully retain that controller-owned transaction.
        transaction ??= record.transaction
        let rollbackVerified = false
        let rollbackFailure = transaction?.recoveryRequired ? failure : undefined
        if (transaction?.mutationStarted && record.applyCommitted !== true && rollbackFailure === undefined) {
          try {
        ...
        if (rollbackFailure !== undefined) {
          ...
          return {
            kind: 'isolated-cell-apply', version: 1, ok: false, planDigest,
            failure: safeCellWorkflowDiagnostic(error(
              transaction.recoveryRequired ? 'CELL_APPLY_RECOVERY_REQUIRED' : 'CELL_APPLY_ROLLBACK_FAILED',
              'Apply failed and safe rollback could not be established; source and recovery state remain preserved.',
  ...
  async function discard(input, call = {}, internal = false) {
    ...
    const record = selected.record
    // Before waiting, only reject already-terminal recovery. An active Apply
    // may still complete or roll back after its lifetime is aborted below.
    if (record.phase === 'rollback-failed' || record.creationFailure !== undefined) assertRecoveryAllowsCleanup(record)
