...
async function ordinaryFilePathMatches(path, expected) {
  try {
    const parent = await inspectOrdinaryDirectoryPhysical(dirname(path))
    if (!parent.stable) return false
    const physical = await realpath(path)
    const info = await lstat(physical, { bigint: true })
    return info.isFile()
      && !info.isSymbolicLink()
      && info.nlink === 1n
      && samePhysicalObjectIdentity(statIdentity(info), expected)
  } catch {
    return false
  }
}
...
async function restoreFileContentInPlace(path, content, expected, observed) {
  ...
  if (!await ordinaryFilePathMatches(path, observed)) {
  ...
  try {
    ...
    if (!current.isFile() || current.isSymbolicLink() || current.nlink !== 1n
        ...
        || !await ordinaryFilePathMatches(path, observed)) {
  ...
  if (!restored.isFile() || restored.isSymbolicLink() || restored.nlink !== 1n
      ...
      || !await ordinaryFilePathMatches(path, expected)) {
