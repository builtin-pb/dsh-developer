...
export async function inspectSession(source, { signal, limit = 20, sourceRoot } = {}) {
  ...
  const counts = { toolCalls: 0, toolResults: 0, failedTools: 0, turnEnds: 0, pendingCalls: 0,
  ...
  const report = {
    ...
    calls: [], errors: [], warnings: [], omissions: { calls: 0, errors: 0, outputBound: false },
  ...
  function entry(turn, step, callId) {
    ...
    if (!entries.has(key)) entries.set(key, { turn, callId: safeText(callId), calls: 0, results: 0, result: null })
...
export function formatSessionReport(report) {
  ...
  for (const call of report.calls) {
    lines.push(`- ${call.name} [${call.callId}] turn ${call.turn}, step ${call.step}: ${call.status}`)
  ...
  if (report.omissions.calls || report.omissions.errors) lines.push(`Omitted: ${report.omissions.calls} calls, ${report.omissions.errors} error samples${report.omissions.outputBound ? ' (output bound)' : ''}. Counts cover all scanned supported events.`)
