...
async function listLayout(root, signal) {
  active(signal)
  const allowedDirectories = new Set(['node_modules', '.dsh-module-fallback'])
  const values = []
...
function validateProfileManifest(file) {
  ...
  exactKeys(profile, new Set(['bundles', 'patchReload']), 'dsh.profile')
  ...
  if (profile.patchReload !== undefined && !['live', 'startup'].includes(profile.patchReload)) failure('PROFILE_MANIFEST_INVALID', 'patchReload must be live or startup.')
  return {
    value,
    evidence: {
      digest: file.digest,
...
async function scanPass(context, dependencies) {
  ...
  const layoutBefore = await listLayout(profileBefore.physicalPath, signal)
  ...
  const manifest = validateProfileManifest(packageFileValue)
  ...
  const layoutAfter = await listLayout(profileBefore.physicalPath, signal)
...
function laneFor(version) {
  ...
  failure('PROFILE_ATTESTATION_LANE_UNSUPPORTED', 'Profile attestation supports exact DSH ' + DSH_COMPATIBILITY_TARGET + ' and ' + DSH_PREVIEW_TARGET + '.', { version }, 'Select one exact reviewed DSH installation and rerun.')
}
