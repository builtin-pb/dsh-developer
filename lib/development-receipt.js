...
export function protectVerificationReceipt(receipt, cases, processProtection) {
  ...
  const hasPrivateKey = findings.includes('private-key') || receipt.privateKeyOutput === true
    || receipt.cases.some(item => item.privateKeyOutput === true) || processProtection === 'private-key'
  const withholdAll = hasPrivateKey || processProtection === 'incomplete-process-output'
  if (!findings.length && !withholdAll) return receipt
  let reason = processProtection ?? 'possible-credentials'
  if (hasPrivateKey) reason = 'private-key'
  ...
  const protectedReceipt = {
    ...
    outputProtection: { withheld: true, reason },
  }
