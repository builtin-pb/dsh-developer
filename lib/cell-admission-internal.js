...
export async function inspectIsolatedCellAdmissionInternal(dshPath, options = {}, dependencies = {}) {
  ...
  const checks = [
    ...
    check(
      ...
      labReport.ok
        ? 'The local execution provider enforces the candidate containment and cleanup prerequisites with fixed keyless fixtures.'
        : 'No verified local boundary is available, so execution-bearing incubation remains unsupported.',
      { provider: labReport.provider, policy: labReport.policy, digest: labReport.evidenceDigest },
    ),
    check(
