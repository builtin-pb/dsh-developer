...
export async function promoteCreatorExport(source, output, options = {}) {
  ...
  try {
    await renameDirectoryExclusive(staging, destinationInfo.destination)
  } catch (error) {
    if (error.code === 'RENAME_STATE_UNKNOWN') {
      throw new DshDeveloperError('COMMIT_STATE_AMBIGUOUS', 'The exclusive rename was not acknowledged; inspect both paths before retrying.', { commitState: 'ambiguous', staging, destination: destinationInfo.destination })
    }
    throw new DshDeveloperError(
