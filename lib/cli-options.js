...
const FLAG_OPTIONS = new Set(['--install-cli', '--json', '--skip-runtime', '--online', '--help', '-h'])
const COMMAND_OPTION_KEYS = new Map([
  ['project', new Set(['source', 'json'])],
  ['knowledge', new Set(['dsh', 'upstream', 'topic', 'package', 'json'])],
  ['session', new Set(['source', 'limit', 'json'])],
  ['run', new Set(['source', 'script', 'scriptArgs', 'timeoutMs', 'json'])],
  ['verify', new Set(['source', 'cases', 'patch', 'dsh', 'profile', 'online', 'timeoutMs', 'json'])],
  ['dev', new Set(['source', 'patch', 'dsh', 'port', 'online', 'timeoutMs', 'json'])],
