...
import { APPLE_CELL_IMAGE } from './lab/apple-container.js'
...
export function requireCellAdmissionGrant(report) {
  ...
  const supported = grant.providerId === 'wsl2-bubblewrap'
    || (grant.providerId === 'apple-container' && grant.distro === APPLE_CELL_IMAGE)
  if (!supported || typeof grant.distro !== 'string' || grant.distro.length === 0) {
