...
export async function inspectSourceMigrationInternal(source, options, dependencies = {}) {
  ...
  const first = await scan(source, { signal: options.signal, excludeDependencies: true, allowBinaryAssets: true })
  ...
  const final = await scan(source, { signal: options.signal, excludeDependencies: true, allowBinaryAssets: true })
