...
export async function inspectSourceMigrationInternal(source, options, dependencies = {}) {
  ...
  const first = await scan(source, { signal: options.signal })
  ...
  const final = await scan(source, { signal: options.signal })
  ...
  const checks = [
    ...
    },
    {
      ...
      evidence: { before: first.fingerprint, after: final.fingerprint, budget: final.budget },
    },
  ]
