...
const CAPABILITIES = Object.freeze([
  {
    ...
    label: 'Native plugin, skill, command, and shell-environment services',
    packages: [
...
async function findDshPackage(invocation) {
...
async function findInstalledPackage(dshPackage, packageName) {
...
async function inspectCapability(spec, dshPackage, runtimeVersion, lane) {
  ...
  for (const name of spec.packages) {
    const installed = await findInstalledPackage(dshPackage, name)
    packages.push(installed ?? { name, missing: true })
  }
...
export async function inspectDshCapabilities(dshPath, options = {}) {
  ...
  const dshPackage = await findDshPackage(invocation)
  if (!dshPackage) {
    checks.push(check(
