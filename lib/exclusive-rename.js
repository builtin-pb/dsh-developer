...
// POSIX uses the same kernel no-replace operation for files and directories.
// Windows rename replaces files, so publish the same inode with an exclusive
// hard link, then remove the private source name. An unacknowledged removal
// leaves both names as recovery evidence; callers must not replay or clean it.
export async function renameFileExclusive(source, destination) {
  if (process.platform !== 'win32') return renameDirectoryExclusive(source, destination)
  await link(source, destination)
  try {
    await unlink(source)
  } catch (cause) {
    throw Object.assign(new Error('Exclusive file publication succeeded but source removal was not acknowledged.'), {
      code: 'RENAME_STATE_UNKNOWN', cause,
    })
  }
}
