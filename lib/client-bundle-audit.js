...
// Exact native frontend seed tables and provider declarations: DSH 0.1.5-rc.2 / 0.1.6-alpha.1.
...
function parseRequire(source, start) {
  ...
  if (source[index] !== '(') return undefined
  index = skipWhitespace(source, index + 1)
  // On one line this is a method/declaration; across a newline it can be a
  // loader call followed by a separate block through semicolon insertion.
  const body = skipWhitespace(source, index + 1)
  if (source[index] === ')' && source[body] === '{'
      && !/[\r\n\u2028\u2029]/u.test(source.slice(start, body))) return undefined
