...
// Exact native frontend seed tables and provider declarations: DSH 0.1.5-rc.2 / 0.1.6-alpha.2.
...
const CORE_CLIENT_SERVICE_OWNERS = Object.freeze(Object.fromEntries(Object.entries({
  ...
  documentPreviews: { release: '@deepseek-ai/dsh-client-ui-sidebar-documentpreview', preview: '@deepseek-ai/dsh-client-ui-sidebar-documentpreview' },
  dynamicCordisRunner: { release: '@deepseek-ai/dsh-cordis-client-runner', preview: '@deepseek-ai/dsh-cordis-client-runner' },
  fileUpload: { release: '@deepseek-ai/dsh-client-file-upload', preview: '@deepseek-ai/dsh-client-file-upload' },
  ...
  resources: { release: '@deepseek-ai/dsh-client-resources', preview: '@deepseek-ai/dsh-client-resources' },
  sessionLogDownload: { release: '@deepseek-ai/dsh-session-log-export', preview: '@deepseek-ai/dsh-session-log-export' },
  sessions: { release: '@deepseek-ai/dsh-api-session-controller', preview: '@deepseek-ai/dsh-api-session-controller' },
  ...
  sidebarRight: { release: '@deepseek-ai/dsh-client-ui-sidebar-right', preview: '@deepseek-ai/dsh-client-ui-sidebar-right' },
  sidebarRightTabs: { release: '@deepseek-ai/dsh-client-ui-sidebar-right', preview: '@deepseek-ai/dsh-client-ui-sidebar-right' },
  slots: { release: '@deepseek-ai/dsh-client-ui-renderer', preview: '@deepseek-ai/dsh-client-ui-renderer' },
  ...
  uiConversation: { release: '@deepseek-ai/dsh-client-ui-conversation', preview: '@deepseek-ai/dsh-client-ui-conversation' },
  uiRenderer: { release: '@deepseek-ai/dsh-client-ui-renderer', preview: '@deepseek-ai/dsh-client-ui-renderer' },
  uiSession: { release: '@deepseek-ai/dsh-client-ui-session', preview: '@deepseek-ai/dsh-client-ui-session' },
  uiWorkspace: { release: '@deepseek-ai/dsh-client-ui-workspace', preview: '@deepseek-ai/dsh-client-ui-workspace' },
  webTerminals: { preview: '@deepseek-ai/dsh-api-terminal-controller' },
  workspaces: { release: '@deepseek-ai/dsh-api-workspace-controller', preview: '@deepseek-ai/dsh-api-workspace-controller' },
...
function parseRegistration(source, start) {
  ...
  let factoryEnd
  if (source[index] === ':') {
    factoryEnd = parseFactory(source, index + 1)
  } else if (source[index] === '(') {
    // Object-method shorthand: factory(require) { ... }. The body brace is the
    // continuation point so require/provide checks still scan inside the factory.
    const parametersEnd = parseFactoryParameters(source, index)
    if (parametersEnd === undefined) return undefined
    const body = skipWhitespace(source, parametersEnd)
...
function scanCode(source, start, state, options = {}) {
  ...
  for (let index = start; index < source.length;) {
    ...
    if (registration) {
      ...
      state.registrationStarts.push(index)
...
function scanBoundaryCalls(source) {
  ...
  return {
    ...
    registrationStarts: state.registrationStarts,
...
function inspectLoaderRequests(ast, registrationStarts) {
  const pending = new Set(registrationStarts), loaders = new Set(), argumentOwners = new Set(), requests = new Set()
  let dynamicRequestCount = 0, unsupportedReferences = 0
  // Only the parameter of the registration actually accepted above belongs to
  // DSH. Nested parameters and unrelated objects named require are ordinary JS.
  traverse(ast, {
    CallExpression(path) {
      if (!pending.has(path.node.start)) return
      const callee = path.node.callee, owner = callee.object, global = owner?.object
      // Chained calls share their start offset with the inner registration.
      // Match the actual loader call before interpreting its object argument.
      if (callee.type !== 'MemberExpression' || callee.computed || callee.property.name !== 'load'
          || owner?.type !== 'MemberExpression' || owner.computed || owner.property.name !== '__ModuleLoader__'
          || global?.type !== 'Identifier' || !['window', 'globalThis'].includes(global.name)
          || path.scope.getBinding(global.name)) return
      const argument = path.get('arguments')[0]
      if (!argument?.isObjectExpression()) return
      const properties = argument.get('properties')
      const named = name => properties.filter(property =>
        property.node.key?.name === name || property.node.key?.value === name)
      if (properties.some(property => property.isSpreadElement() || property.node.computed)
          || named('factory').length !== 1 || named('id').length !== 1) {
        fail('CLIENT_BUNDLE_REGISTRATION_INVALID',
          'Client registration must have one unambiguous id and factory, without computed or spread properties.')
      }
      const factory = named('factory')[0]
      const fn = factory.isObjectMethod() ? factory : factory.get('value')
      if (!fn.isFunctionExpression() && !fn.isArrowFunctionExpression() && !fn.isObjectMethod()) {
        fail('CLIENT_BUNDLE_REGISTRATION_INVALID', 'Client registration factory must be a direct function, arrow or method.')
      }
      const binding = fn.scope.getOwnBinding('require')
      if (binding?.kind === 'param') loaders.add(binding)
      if (!fn.isArrowFunctionExpression()) argumentOwners.add(fn.node)
      pending.delete(path.node.start)
    },
  })
  if (pending.size > 0) {
    fail('CLIENT_BUNDLE_REGISTRATION_INVALID', 'Client registration must call the browser module loader directly.')
  }
  for (const binding of loaders) {
    unsupportedReferences += binding.constantViolations.filter(path => {
      // `var require;` redeclares the parameter without assigning it. Loop
      // targets and initialized declarations can replace the supplied loader.
      return !path.isVariableDeclarator() || path.node.init !== null
        || !path.parentPath.isVariableDeclaration({ kind: 'var' })
        || ['ForInStatement', 'ForOfStatement'].includes(path.parentPath.parentPath.node.type)
    }).length
  }
  traverse(ast, {
    ReferencedIdentifier(path) {
      if (path.node.name === 'arguments') {
        // A computed method key runs in its enclosing scope, not the method
        // body. Arrow scopes inherit arguments unless they bind it themselves.
        let owner = path.scope.getFunctionParent()
        while (owner?.path.isArrowFunctionExpression()) owner = owner.parent.getFunctionParent()
        const binding = path.scope.getBinding('arguments')
        if (argumentOwners.has(owner?.path.node)
            && (!binding || (binding.scope === owner && ['var', 'local'].includes(binding.kind)))) {
      ...
      if (binding && !loaders.has(binding)) return
      const parent = path.parentPath
      if (binding && parent.isCallExpression() && parent.node.callee === path.node && !parent.node.optional) {
        const args = parent.node.arguments
        if (args.length === 1 && args[0].type === 'StringLiteral') requests.add(args[0].value)
        else dynamicRequestCount += 1
      } else if (!parent.isUnaryExpression({ operator: 'typeof' })) {
        // Async/member access, aliases, destructuring, passing or returning the
        // loader can execute requests beyond this entry. Do not certify chunks
        // merely because their names or bytes are available in the source tree.
        unsupportedReferences += 1
      }
    },
  })
  if (unsupportedReferences > 0) {
    fail('CLIENT_BUNDLE_UNSUPPORTED_LOADER',
      'The browser bundle uses async chunks or indirect loader references that this audit cannot verify. '
        + 'Emit a single client bundle using direct require("module") calls.',
      { references: unsupportedReferences })
  }
  return { requests: [...requests].sort((left, right) => left.localeCompare(right, 'en')), dynamicRequestCount }
}
...
export function inspectClientBundle(files, packageValue) {
  ...
  try {
    ...
    ast = parse(source, { sourceType: 'script', attachComment: false })
  } catch (error) {
    if (error instanceof RangeError) {
      fail('CLIENT_BUNDLE_ANALYSIS_LIMIT', 'Client script complexity exceeds the static audit parser limit.', { path: clientPath })
    }
  ...
  let loaderRequests
  try {
    loaderRequests = inspectLoaderRequests(ast, scanned.registrationStarts)
  } catch (error) {
    if (!(error instanceof RangeError)) throw error
    fail('CLIENT_BUNDLE_ANALYSIS_LIMIT', 'Client script complexity exceeds the static loader audit limit.', { path: clientPath })
  }
  const { requests, dynamicRequestCount } = loaderRequests
