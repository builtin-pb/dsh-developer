...
import { formatHookBridgeReport, inspectHookBridge } from './hook-bridge-doctor.js'
...
import { agentProjectRoot, formatDoctorReport, resolveNativeSource } from './native-tool-internal.js'

/** @param {string} source @param {import("@deepseek-ai/dsh-commands").CommandInvocation} invocation */
function commandSource(source, invocation) {
  return resolveNativeSource(source, invocation.agent, { allowLaunchDirectory: true })
}
 ...
 * @param {K[]} required
 * @returns {Record<K, string> & Record<string, unknown>}
 */
function parseInput(rawInput, allowed, required) {
  let value
  try {
    value = /** @type {unknown} */ (JSON.parse(rawInput.trim()))
  } catch {
  ...
  return /** @type {Record<K, string> & Record<string, unknown>} */ (input)
}
...
/** @param {import("@deepseek-ai/dsh-commands").CommandInvocation} invocation @returns {Promise<import("@deepseek-ai/dsh-commands").CommandResult>} */
async function doctorCommand(invocation) {
  try {
    ...
    const report = await doctorSource(commandSource(input.source, invocation), {
...
/** @param {import("@deepseek-ai/dsh-commands").CommandInvocation} invocation @returns {Promise<import("@deepseek-ai/dsh-commands").CommandResult>} */
async function hookDoctorCommand(invocation) {
  try {
    const input = parseInput(invocation.rawInput, new Set(['source', 'dialect']), ['source', 'dialect'])
    if (input.dialect !== 'codex' && input.dialect !== 'claude-code') {
      throw new DshDeveloperError('COMMAND_USAGE', 'dialect must be "codex" or "claude-code".')
    }
    const report = await inspectHookBridge(input.source, {
      dialect: input.dialect,
      dshPath: currentDshEntry(),
      // Native commands are an explicit human command surface. Prefer an attached
      // Agent project when present; otherwise confine relative paths to launch cwd.
      sourceRoot: agentProjectRoot(invocation.agent, { allowLaunchDirectory: true }),
      signal: invocation.signal,
    })
    return { kind: report.ok ? 'success' : 'error', text: formatHookBridgeReport(report) }
  } catch (error) {
    const diagnostic = asDiagnostic(error)
    return { kind: 'error', text: diagnostic.code + ': ' + diagnostic.message }
  }
...
/** @param {import("@deepseek-ai/dsh-tools").ToolRuntime} tools @param {import("@deepseek-ai/dsh-commands").CommandInvocation} invocation @returns {Promise<import("@deepseek-ai/dsh-commands").CommandResult>} */
async function uiCapabilitiesCommand(tools, invocation) {
  try {
    const input = parseInput(invocation.rawInput, new Set(), [])
    if (Object.keys(input).length !== 0) {
      throw new DshDeveloperError('COMMAND_USAGE', 'UI capability inspection accepts an empty JSON object.')
    }
    const report = inspectUiCapabilities(tools.schemas(invocation.agent), {
...
async function compatibilityCommand(invocation) {
  try {
    ...
    const report = await inspectCompatibilityMatrix(commandSource(input.source, invocation), {
...
async function impactCommand(invocation) {
  try {
    ...
    const report = await inspectUpstreamImpact(commandSource(input.source, invocation), {
      releaseDsh: input.releaseDsh ?? currentDshEntry(),
      previewDsh: input.previewDsh,
      signal: invocation.signal,
    })
...
async function preflightCommand(invocation) {
  try {
    ...
    const report = await inspectProfilePreflight(commandSource(input.source, invocation), {
      dshPath: currentDshEntry(),
...
async function promoteCommand(invocation) {
  try {
    ...
    const result = await promoteCreatorExport(commandSource(input.source, invocation), commandSource(input.output, invocation), {
      dshPath: currentDshEntry(),
      signal: invocation.signal,
    })
    return {
    ...
    const diagnostic = asDiagnostic(error)
    const staging = 'staging' in diagnostic && typeof diagnostic.staging === 'string' ? '\nRetained staging: ' + diagnostic.staging : ''
    return { kind: 'error', text: diagnostic.code + ': ' + diagnostic.message + staging }
  }
}

/** @param {{commands: import("@deepseek-ai/dsh-commands").CommandRuntime, tools: import("@deepseek-ai/dsh-tools").ToolRuntime}} dependencies */
export function registerNativeCommandsWithDependencies({ commands, tools }) {
  commands.register({
  ...
  commands.register({
    name: 'dsh-developer-hook-doctor',
    description: 'Statically classify one Codex or Claude Code hook config against the exact running DSH bridge bytes',
    input: { hint: '{"source":"<project-file>","dialect":"codex"}' },
    handler: hookDoctorCommand,
  })
  commands.register({
  ...
  commands.register({
    ...
    input: { hint: '{}' },
    handler: (invocation) => uiCapabilitiesCommand(tools, invocation),
  })
}

/** @param {import("@deepseek-ai/cordis").Context} ctx */
export function registerNativeCommands(ctx) {
  return registerNativeCommandsWithDependencies({
