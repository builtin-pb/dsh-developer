...
export function resolveAuthorityState(ctx, agent) {
  ...
  const approvalPolicy = loggedApproval ?? serviceValue(
    () => ctx?.get?.('approval')?.overrideOf?.(session) ?? ctx?.get?.('approval')?.config?.policy,
  )
  const reasons = [
...
export function inspectAuthoritySafety(ctx, agent, value) {
  const state = resolveAuthorityState(ctx, agent)
  ...
  const disposeAll = (disposers) => {
    for (const dispose of disposers.reverse()) {
      try {
        ...
        ctx.logger?.warn?.('dsh-developer: fixed-authority disposer failed: ' + (error?.message ?? String(error)))
      }
    }
  }
    ...
    mutateTools(() => {
      ...
      for (const name of TARGET_TOOLS) {
        ...
        try {
          ...
          ctx.logger?.warn?.('dsh-developer: could not correct fixed-authority ' + name + ' schema: ' + error.message)
        }
      }
      if (uncorrected.size > 0) {
        try {
          ...
          ctx.logger?.warn?.('dsh-developer: could not guard uncorrected fixed-authority tools: '
...
export function registerDelegationSafety(ctx) {
  return registerAuthoritySafety(ctx)
}
